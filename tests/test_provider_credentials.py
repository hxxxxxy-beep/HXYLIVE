...
class ProviderCredentialTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.db = Database(Path(self._tmpdir.name) / "hxylive.db")
    ...
    async def test_provider_login_state_verify_clears_stale_connected_flag(self):
        ...
        await store.save(
            ...
            username="alice",
            is_logged_in=True,
            cookies=[{"name": "auth-token", "value": "stale", "domain": ".twitch.tv", "path": "/"}],
        )

        class Provider:
            capabilities = ProviderCapabilities(can_login=True, can_sync_following=True)
            auth = None
            session_store = store
        ...
        original_for = app_main._provider_for
        original_db = app_main.db
        app_main.db = self.db
        app_main._provider_for = lambda source_type: Provider()
        try:
            status = await app_main._provider_login_state("twitch", verify=True)
        finally:
            app_main._provider_for = original_for
            app_main.db = original_db

        self.assertFalse(status["isLoggedIn"])
        self.assertIn("not logged in", (status.get("lastError") or "").lower())
        row = await self.db.get_provider_session("twitch")
        self.assertFalse(row["is_logged_in"])

    async def test_invalidate_provider_login_marks_session_logged_out(self):
        store = ProviderSessionStore(self.db)
        await store.save(
            "twitch",
            username="alice",
        ...
        app_main._provider_for = lambda source_type: Provider()
        try:
            await app_main._invalidate_provider_login("twitch", "Twitch login required")
        finally:
            app_main._provider_for = original_for

        row = await self.db.get_provider_session("twitch")
        self.assertFalse(row["is_logged_in"])
        self.assertEqual("Twitch login required", row["last_error"])
