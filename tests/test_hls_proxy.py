...
class HlsProxyTests(unittest.TestCase):
    ...
    def test_ffmpeg_input_uses_upstream_for_regular_hls(self):
        stream = main.ResolvedStream(
        ...
        # Must not loop ffmpeg through the same uvicorn HLS proxy.
        self.assertEqual(stream.url, url)
        self.assertEqual({"Referer": "https://example.test/model"}, headers)
        self.assertEqual(stream.url, source_url)
        self.assertEqual(0, len(main._HLS_PROXY_CACHE))

    def test_ffmpeg_input_uses_upstream_for_chaturbate_hls(self):
        stream = main.ResolvedStream(
        ...
        self.assertEqual(stream.url, url)
        self.assertEqual({"Referer": "https://chaturbate.com/"}, headers)
        self.assertEqual(stream.url, source_url)
        self.assertEqual(0, len(main._HLS_PROXY_CACHE))

    def test_ffmpeg_input_proxies_preloaded_master_for_header_auth(self):
        stream = main.ResolvedStream(
        ...
        # FFmpeg 7 cannot attach HTTP headers to local file inputs, so prefetched
        # masters must go through the HLS proxy (headers applied server-side).
        self.assertTrue(url.startswith("http://127.0.0.1:"))
        self.assertIn("/api/proxy/hls/", url)
        self.assertIsNone(headers)
        self.assertEqual(stream.url, source_url)
        self.assertGreaterEqual(len(main._HLS_PROXY_CACHE), 1)
        token = url.rsplit("/api/proxy/hls/", 1)[-1].split(".", 1)[0]
        body = main._HLS_PROXY_CACHE[token]["body"].decode("utf-8")
        self.assertIn("/api/proxy/hls/", body)
        self.assertNotIn(
            "https://edge.mmcdn.com/live/chunklist_3_video_llhls.m3u8",
            body,
        )
