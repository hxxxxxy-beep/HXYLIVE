...
class DiscoverProviderRegistryTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_youtube_search_keeps_offline_channel_and_strips_at(self):
        """YouTube offline / @handle search hits must surface in Discover."""
        registry = _Registry()
        registry.providers = {
            "youtube": _Provider("youtube", can_follow=True, models=[
                {
                    "username": "kurzgesagt",
        ...
        names = [item["username"] for item in result["models"]]
        self.assertEqual(["kurzgesagt"], names)
        self.assertFalse(result["models"][0]["is_online"])
        self.assertEqual("offline", result["models"][0]["room_status"])
        self.assertFalse(result["models"][0]["stream_available"])

    async def test_youtube_url_search_keeps_offline_channel(self):
        """Pasted youtube.com/@handle URLs must normalize before Discover filters."""
        registry = _Registry()
        registry.providers = {
            "youtube": _Provider("youtube", can_follow=True, models=[
                {
                    "username": "angeliclofiasmr",
                    "display_name": "Angelic Lofi ASMR",
                    "user_id": "UCXHIwO5ze7cKuqgs5u8XQUQ",
                    "is_online": False,
                    "room_status": "offline",
                    "viewers": 0,
                    "tags": [],
        ...
        result = await discover.discover_models(
            ...
            gender=None,
            live_query=None,
            search="https://www.youtube.com/@AngelicLofiASMR",
            tags=None,
            sort="viewers",
        )
        self.assertEqual(["angeliclofiasmr"], [item["username"] for item in result["models"]])
        self.assertFalse(result["models"][0]["is_online"])
