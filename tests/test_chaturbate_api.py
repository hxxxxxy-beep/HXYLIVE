...
class _FakeFlareSolverr:
    ...
    async def solve_challenge(self, url, *, force=False):
        self.urls.append(url)
        self.force_flags.append(force)
        return {
            "cookies": {"cf_clearance": "solved", "__cf_bm": "solved-bm"},
...
class ChaturbateRoomlistTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_followed_sync_attempts_when_sessionid_present_even_if_unverified(self):
        from app.following_sync import FollowedSyncResult

        api = ChaturbateAPI(_UnverifiedCookieAuth())
        api._fetch_followed_roomlist_api = AsyncMock(
            return_value=FollowedSyncResult(
                [{"username": "alice"}],
                trusted=True,
                authoritative=True,
            )
        )
        api._fetch_followed_html = AsyncMock()

        result = await api.get_followed_models()

        self.assertTrue(result.trusted)
        self.assertEqual(result[0]["username"], "alice")
        api._fetch_followed_roomlist_api.assert_awaited_once()

    async def test_followed_sync_requires_sessionid_cookie(self):
    ...
    async def test_validate_session_retries_403_via_flaresolverr(self):
        flare = _FakeFlareSolverr()
        auth = ChaturbateAuthService(db=object(), flaresolverr=flare)
        auth._cookies = {
            "sessionid": "real-session",
            "csrftoken": "real-csrf",
            "cf_clearance": "mac-bound",
            "__cf_bm": "mac-bound-bm",
        }
        statuses = [403, 200]
        ...
        class _StatusSession:
            ...
            def get(self, *args, **kwargs):
                return _StatusResponse(statuses.pop(0))

        with patch(
            "app.services.chaturbate_auth.aiohttp_client_session",
            return_value=_StatusSession(),
        ):
            ok, reason = await auth._validate_session_detail()

        self.assertTrue(ok)
        self.assertIsNone(reason)
        self.assertEqual(["https://chaturbate.com/api/ts/chatmessages/pm_users/?offset=0"], flare.urls)
        self.assertEqual([True], flare.force_flags)
        self.assertEqual("real-session", auth._cookies["sessionid"])
        self.assertEqual("solved", auth._cookies["cf_clearance"])
        self.assertEqual("solved-bm", auth._cookies["__cf_bm"])
        self.assertEqual("Solved-UA", auth._user_agent)

    async def test_validate_session_403_without_flaresolverr_explains_cloudflare(self):
        auth = ChaturbateAuthService(db=object(), flaresolverr=None)
        auth._cookies = {"sessionid": "real-session", "csrftoken": "real-csrf"}
        ...
        self.assertFalse(ok)
        self.assertIn("Cloudflare", reason or "")
        self.assertIn("FlareSolverr", reason or "")
    ...
    async def test_roomlist_retries_without_cookies_after_login_redirect(self):
        auth = _MutableCookieAuth()
        auth._cookies = {"sessionid": "stale", "csrftoken": "x"}

        class _RedirectThenOk(ChaturbateAPI):
            def __init__(self):
                super().__init__(auth)
                self.calls = []
            ...
            async def _enrich_models_profile_images(self, models):
                return None
            ...
            async def _request(self, method, url, headers=None, **kwargs):
                ...
                if headers.get("Cookie"):
                    return _FakeResponse(302, b"", {"Location": "/auth/login/"}, "text/html")
                return _json_response({
                    "rooms": [{"username": "alice", "current_show": "public", "num_users": 3}],
                    "total_count": 1,
                })

            async def _scrape_live_models(self, page, limit, gender, search):
                raise AssertionError("should not scrape after guest retry succeeds")

        api = _RedirectThenOk()
        result = await api.get_live_models(page=1, limit=12)
        self.assertEqual(1, len(result["models"]))
        self.assertEqual("alice", result["models"][0]["username"])
        self.assertEqual(2, len(api.calls))
        self.assertIn("Cookie", api.calls[0])
        self.assertNotIn("Cookie", api.calls[1])

    async def test_roomlist_keeps_models_when_avatar_enrichment_fails(self):
        # Discover no longer awaits avatar enrichment on the roomlist path.
        api = _RoomlistAPI(_json_response({
            "rooms": [{"username": "alice", "current_show": "public", "num_users": 2}],
            "total_count": 1,
        }))

        async def boom(_models):
            raise RuntimeError("enrichment failed")

        api._enrich_models_profile_images = boom
        result = await api.get_live_models(page=1, limit=12)
        self.assertEqual(1, len(result["models"]))
        self.assertEqual("alice", result["models"][0]["username"])

    async def test_roomlist_does_not_block_on_profile_image_enrichment(self):
        api = _RoomlistAPI(_json_response({
            "rooms": [
                {
                    "username": "alice",
        ...
        api._enrich_models_profile_images = AsyncMock(
            side_effect=AssertionError("discover must not await avatar enrichment")
        )
        result = await api.get_live_models(page=1, limit=1)
        self.assertEqual("alice", result["models"][0]["username"])
        self.assertEqual("", result["models"][0].get("profile_image_url") or "")
        api._enrich_models_profile_images.assert_not_awaited()
        api._enrich_models_profile_images.assert_not_called()
    ...
    async def test_roomlist_parses_current_chaturbate_fields(self):
        ...
        # Roomlist `img` stays the live cover; circular avatar stays empty until
        # summary_card_image enrichment finds a real self-set face (never /ri/).
    ...
    async def test_followed_models_prefers_roomlist_and_combines_online_offline(self):
        ...
        self.assertEqual("csrf", api.requests[0]["headers"]["X-CSRFToken"])

    async def test_followed_models_empty_roomlist_falls_back_to_html(self):
        html = """
        <html><body>
          <a href="/followed-cams/">Followed Cams</a>
          <li class="room_list_room" data-room="alice">
            <a href="/alice/"><img data-src="//thumb.example.test/alice.jpg" alt="Alice"></a>
            <span class="cams">12</span>
          </li>
        </body></html>
        """
        api = _FollowedAPI([
        ...
        self.assertTrue(all("/api/ts/roomlist/" in request["url"] for request in api.requests[:2]))
        self.assertEqual("https://chaturbate.com/followed-cams/", api.requests[2]["url"])

    async def test_followed_html_fallback_retries_cloudflare_challenge(self):
        challenge = """
        <html><body>Cloudflare cf-browser-verification challenge</body></html>
        """
        html = """
        <html><body>
          <a href="/followed-cams/">Followed Cams</a>
        ...
        api = _FollowedAPI([
            _empty_followed_response(),
            _empty_followed_response(),
            _FakeResponse(200, challenge.encode("utf-8"), {}, "text/html"),
            _FakeResponse(200, html.encode("utf-8"), {}, "text/html"),
        ], flaresolverr=flare)
        ...
        self.assertTrue(result.trusted)
        self.assertEqual(["alice"], [item["username"] for item in result])
        self.assertEqual(["https://chaturbate.com/followed-cams/"], flare.urls)
        self.assertEqual(4, len(api.requests))

    async def test_followed_html_fallback_reports_login_page(self):
        html = """
        <html><body>
        ...
        self.assertFalse(result.trusted)
        self.assertIn("followed roomlist is empty", result.skipped_reason)
        self.assertIn("login page", result.skipped_reason)
    ...
    async def test_lookup_username_empty_summary_leaves_profile_image_empty(self):
        payload = {
            "broadcaster_username": "assuri_love",
            "room_status": "public",
            "room_title": "live",
            "num_users": 40,
            "hls_source": "https://edge.test/live.m3u8",
        ...
        api = ChaturbateAPI(_FakeAuth())
        api._request = AsyncMock(return_value=_json_response(payload))
        model = await api.lookup_username("assuri_love")
        self.assertTrue(model["is_online"])
        self.assertEqual(
            "https://thumb.live.mmcdn.com/riw/assuri_love.jpg",
            model["thumbnail"],
        )
        # Room posters look like live screenshots — never circular faces.
        self.assertEqual("", model["profile_image_url"])

    async def test_resolve_profile_images_skips_room_poster_fallback(self):
        api = ChaturbateAPI(_FakeAuth())
        api._chatvideocontext_summary_image = AsyncMock(return_value="")
        images = await api.resolve_profile_images(["Assuri_Love", "missing"])
        self.assertEqual({}, images)

    async def test_durable_profile_image_rejects_room_poster(self):
        self.assertEqual(
            "",
            ChaturbateAPI._durable_profile_image(
                "alice",
                "https://thumb.live.mmcdn.com/ri/alice.jpg",
            ),
        )
        self.assertEqual(
    ...
    async def test_roomlist_skips_cached_profile_image_lookup_on_discover(self):
        api = _RoomlistAPI(_json_response({
            "rooms": [
                {
                    "username": "alice",
        ...
        api.resolve_profile_images = AsyncMock()
        result = await api.get_live_models(page=1, limit=1)
        # Discover returns the live roomlist immediately; avatars hydrate via
        # /api/discover/profile-images (which still uses the profile cache).
        self.assertEqual("", result["models"][0].get("profile_image_url") or "")
        api.lookup_username.assert_not_awaited()
        api.resolve_profile_images.assert_not_awaited()
    ...
    async def test_profile_image_helper_still_enriches_when_called(self):
        api = _RoomlistAPI(_json_response({
            "rooms": [
                {
                    "username": "alice",
        ...
        self.assertEqual("", result["models"][0].get("profile_image_url") or "")
        await api._enrich_models_profile_images(result["models"])
        self.assertEqual(
            "https://s3pv.highwebmedia.com/uploads/photos/alice.jpg",
