...
class BilibiliCategoriesServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_parent_areas_returns_curated_allowlist(self):
        ...
        self.assertEqual(
            [a["parent_area_id"] for a in areas],
            ["9"],
        )
        self.assertNotIn("网游", [a["name"] for a in areas])
        self.assertNotIn("电台", [a["name"] for a in areas])

    def test_curated_constant_order(self):
        rows = curated_bilibili_parent_areas()
        self.assertEqual(
            ["Virtual streamers"],
            [r["name"] for r in rows],
        )
        self.assertTrue(is_curated_bilibili_parent_area_id("9"))
        self.assertFalse(is_curated_bilibili_parent_area_id("5"))
        self.assertFalse(is_curated_bilibili_parent_area_id("2"))
        self.assertFalse(is_curated_bilibili_parent_area_id("1"))
...
class BilibiliCatalogTests(unittest.TestCase):
    ...
    def test_payload_injects_parent_areas_not_gender(self):
        ...
        self.assertNotIn("parent_area:1", keys)
        self.assertNotIn("parent_area:5", keys)
    ...
    def test_payload_uses_curated_allowlist_when_areas_missing(self):
...
class BilibiliProviderTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_all_catalogue_walks_parent_areas_for_depth(self):
        ...
        async def fake_fetch(*, page_no, page_size, parent_area_id, area_id="0"):
            if parent_area_id == "9" and page_no == 1:
                return parent_a
            if parent_area_id == "9":
                return []  # exhaust Virtual streamers
        ...
        with patch(
            ...
            AsyncMock(
                return_value=[
                    {"parent_area_id": "9", "name": "Virtual streamers"},
                    {"parent_area_id": "2", "name": "PC games"},
                ]
            ),
        ), patch.object(
    ...
    async def test_list_live_models_resolves_tag_to_child_area(self):
        ...
        with patch(
            ...
            result = await self.provider.list_live_models(
                page=1, limit=24, tags=["虚拟日常"]
            )
        self.assertEqual(len(result["models"]), 2)
        self.assertEqual(result["models"][0]["tags"], ["虚拟主播", "虚拟日常"])
        fetch.assert_awaited()
        kwargs = fetch.await_args.kwargs
        self.assertEqual(kwargs["parent_area_id"], "9")
        self.assertEqual(kwargs["area_id"], "371")

    async def test_list_live_models_ignores_tag_outside_allowlist(self):
        ...
        with patch(
            ...
            await self.provider.list_live_models(
                page=1, limit=24, tags=["英雄联盟"], parent_area_id="9"
            )
        kwargs = fetch.await_args.kwargs
        self.assertEqual(kwargs["parent_area_id"], "9")
        self.assertEqual(kwargs["area_id"], "0")
...
class BilibiliFollowingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_verify_logged_in_false_without_cookies(self):
        provider = _bili_provider()
        self.assertFalse(await provider.verify_logged_in())

    async def test_verify_logged_in_true_when_nav_ok(self):
        provider = _bili_provider()
    ...
    async def test_sync_following_uses_complete_relation_list(self):
        provider = _bili_provider()
        await provider.session_store.save(
            "bilibili",
            username="me",
        ...
        with patch.object(
            ...
            provider,
            "_bili_sync_relation_followings",
            AsyncMock(return_value=[
                {
                    "username": "111",
                    "display_name": "A",
            ...
            AsyncMock(return_value=[
                {
                    ...
                    "viewers": 88,
                    "room_status": "public",
                    "profile_image_url": "https://i0.hdslb.com/face.jpg",
                    "thumbnail_url": "https://i0.hdslb.com/face.jpg",
                },
            ]),
        ):
            items = await provider.sync_following()
        self.assertEqual({"111", "222"}, {item["username"] for item in items})
        live = next(item for item in items if item["username"] == "222")
        self.assertTrue(live["is_online"])
        self.assertEqual(88, live["viewers"])
        offline = next(item for item in items if item["username"] == "111")
        self.assertFalse(offline["is_online"])
