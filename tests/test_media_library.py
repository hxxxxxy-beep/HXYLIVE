...
class MediaLibraryApiTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.original_staging_dir = app_main.STAGING_DIR
        ...
        app_main.STAGING_DIR = self.output_dir / "staging"
        app_main.STAGING_DIR.mkdir(parents=True, exist_ok=True)
        (app_main.STAGING_DIR / "records").mkdir(parents=True, exist_ok=True)
        app_main.PROFILE_IMAGES_DIR = self.output_dir / "profile-images"
        app_main.db = Database(self.output_dir / "hxylive.db")
        ...
        app_main._mac_download_public_base = ""
        ...
        concat_queue.reset()
        app_main._library_space_blocked = False
        app_main._staging_blocked = False
        app_main._disk_convert_paused = False
        self.original_thumb_wait = app_main.MAC_HELPER_THUMB_WAIT_SECONDS
        app_main.MAC_HELPER_THUMB_WAIT_SECONDS = 0

    async def asyncTearDown(self):
        app_main.db = self.original_db
        app_main.OUTPUT_DIR = self.original_output_dir
        app_main.STAGING_DIR = self.original_staging_dir
        ...
        app_main.MAC_HELPER_THUMB_WAIT_SECONDS = self.original_thumb_wait
        self.tmpdir.cleanup()
    ...
    async def test_lists_videos_and_photos_from_records_folder(self):
        ...
        self.assertIn("model", profiles)
        self.assertEqual(profiles["model"]["latestTitle"], "photo")
        self.assertEqual(
            profiles["model"]["latestRecordingCover"],
            "/api/recording-thumbnail/model/clip.jpg",
        )
        self.assertEqual(profiles["model"]["profileImageUrl"], "")
        self.assertNotIn("thumbnail", profiles["model"])
        ...
        self.assertEqual(items["clip.mp4"]["resolution"], "1920x1080")
        ...
        self.assertNotIn("diskTotalBytes", storage)
        self.assertNotIn("diskTotalFormatted", storage)
        self.assertNotIn("diskUsedFormatted", storage)
        self.assertNotIn("diskUsableFormatted", storage)
        self.assertNotIn("recordingFolderFormatted", storage)
        ...
        self.assertGreaterEqual(storage["diskUsableBytes"], 0)
        self.assertGreaterEqual(storage["diskUsedPercent"], 0)
        self.assertGreaterEqual(storage["diskFreePercent"], 0)

    def test_media_storage_stats_uses_disk_used_not_total_minus_free(self):
        # Reserved root blocks make total-free larger than used; panel must match df used.
        # Percents use usable (= used + free), not filesystem total.
        from collections import namedtuple

        usage = namedtuple("usage", "total used free")
        # 58GiB total, 11GiB used, ~2.9GiB reserved → free(bavail) leaves total-free≈13.9GiB
        total = 58 * 1024**3
        used = 11 * 1024**3
        reserved = int(2.9 * 1024**3)
        free = total - used - reserved
        usable = used + free
        self.assertGreater(total - free, used)

        with patch.object(app_main.shutil, "disk_usage", return_value=usage(total, used, free)):
            storage = app_main._media_storage_stats([])

        self.assertEqual(storage["diskUsedBytes"], used)
        self.assertEqual(storage["diskFreeBytes"], free)
        self.assertNotIn("diskTotalBytes", storage)
        self.assertEqual(storage["diskUsableBytes"], usable)
        self.assertEqual(storage["diskUsedPercent"], round((used / usable) * 100, 1))
        self.assertEqual(storage["diskFreePercent"], round((free / usable) * 100, 1))
        self.assertNotEqual(storage["diskUsedBytes"], total - free)
        self.assertNotEqual(storage["diskUsedPercent"], round((used / total) * 100, 1))

    async def test_lazy_media_library_returns_stored_resolution(self):
        response = self.client.get("/api/media-library?metadata=lazy&kind=video&limit=50")
        self.assertEqual(response.status_code, 200)
        items = {item["filename"]: item for item in response.json()["items"]}
        self.assertEqual(items["clip.mp4"]["resolution"], "1920x1080")

    async def test_set_recording_resolution_persists_for_lazy_scan(self):
        await app_main.db.set_recording_resolution("model", "clip.mp4", "1280x720")
        with patch.object(
            app_main,
            "_media_video_stream_meta",
            new=AsyncMock(return_value={"resolution": "999x999", "fps": 60.0, "bitrate": 1}),
        ):
            response = self.client.get("/api/media-library?metadata=lazy&kind=video&limit=50")
        self.assertEqual(response.status_code, 200)
        items = {item["filename"]: item for item in response.json()["items"]}
        self.assertEqual(items["clip.mp4"]["resolution"], "1280x720")

    async def test_lazy_media_library_returns_probed_fps_and_bitrate(self):
        await app_main.db.set_recording_stream_meta(
            "model",
            "clip.mp4",
            resolution="1920x1080",
            fps=29.97,
        ...
        with patch.object(
            app_main,
            "_media_video_stream_meta",
            new=AsyncMock(return_value={"resolution": "1x1", "fps": 1.0, "bitrate": 1}),
        ):
            response = self.client.get("/api/media-library?metadata=lazy&kind=video&limit=50")
        self.assertEqual(response.status_code, 200)
        items = {item["filename"]: item for item in response.json()["items"]}
        self.assertEqual(items["clip.mp4"]["fps"], 29.97)
        self.assertEqual(items["clip.mp4"]["bitrate"], 4_500_000)
    ...
    async def test_mac_snapshot_and_download_job(self):
        ...
        self.assertEqual(created.json()["method"], "chrome")
        ...
        self.assertTrue(entry["relativePath"].startswith("model(chaturbate)/"))
        self.assertIn("/", entry["relativePath"])
        # Recording projects: model(source)/.projects/<projectId>/YYYY-MM-DD HH-MM-SS[_partNNN].ext
        self.assertRegex(
            entry["relativePath"],
            r"^model\(chaturbate\)/\.projects/[^/]+/\d{4}-\d{2}-\d{2} \d{2}-\d{2}-\d{2}(?:_part\d{3,})?\.mp4$",
        )
    ...
    async def test_mac_download_uses_bilibili_display_name_for_folder(self):
        room_id = "1883358196"
        display_name = "示例主播"
        room_dir = self.output_dir / "records" / room_id
        room_dir.mkdir(parents=True, exist_ok=True)
        clip = room_dir / "live.mp4"
        ...
        await app_main.db.upsert_media_profile(room_id, {"display_name": display_name})
        ...
        self.assertEqual(
            display_name,
            app_main._mac_download_folder_label(room_id, display_name),
        )
        self.assertEqual(
            "tiffy",
            app_main._mac_download_folder_label("tiffy", "Tiffy"),
        )

        library = self.client.get("/api/media-library?kind=video").json()
        item = next(entry for entry in library["items"] if entry["filename"] == "live.mp4")
        created = self.client.post(
            "/api/mac/download-jobs",
            json={"localSessionId": "mac-session", "itemIds": [item["id"]], "method": "motrix"},
        )
        self.assertEqual(created.status_code, 200)
        job_id = created.json()["jobId"]
        job = self.client.get(
        ...
        self.assertEqual(job.status_code, 200)
        relative = job.json()["items"][0]["relativePath"]
        self.assertTrue(relative.startswith(f"{display_name}(bilibili)/"), relative)

        renames = await app_main._mac_numeric_folder_rename_commands([
            {"filename": f"{room_id}(bilibili)/2026-09-05 20-24-23.mp4"},
        ])
        self.assertEqual(1, len(renames))
        self.assertEqual("rename-folder", renames[0]["type"])
        self.assertEqual(f"{room_id}(bilibili)", renames[0]["fromFolder"])
        self.assertEqual(f"{display_name}(bilibili)", renames[0]["toFolder"])

    def test_mac_download_relative_path_keeps_segment_part_suffix(self):
        """Segmented VPS files must not collapse to one Mac path."""
        used = set()
        part1 = app_main._mac_download_relative_path(
            "sunsetgaiaasmr",
            "20260910_105315_fa13bc(twitch)_part001.mp4",
            1789018395,
            used,
            source_type="twitch",
        )
        part2 = app_main._mac_download_relative_path(
            "sunsetgaiaasmr",
            "20260910_105315_fa13bc(twitch)_part002.mp4",
            1789018395,
            used,
            source_type="twitch",
        )
        plain = app_main._mac_download_relative_path(
            "yawn",
            "20260909_065123_4fc38c(twitch).mp4",
            1788917483,
            set(),
            source_type="twitch",
        )
        self.assertEqual(
            part1,
            "sunsetgaiaasmr(twitch)/2026-09-10 10-53-15_part001.mp4",
        )
        self.assertEqual(
            part2,
            "sunsetgaiaasmr(twitch)/2026-09-10 10-53-15_part002.mp4",
        )
        self.assertNotEqual(part1, part2)
        self.assertEqual(plain, "yawn(twitch)/2026-09-09 06-51-23.mp4")

        # Sequential syncs (empty used_paths each time) must still stay unique.
        again = app_main._mac_download_relative_path(
            "sunsetgaiaasmr",
            "20260910_105315_fa13bc(twitch)_part002.mp4",
            1789018395,
            set(),
        ...
        self.assertEqual(again, part2)

    async def test_mac_snapshot_purges_synced_vps_copy(self):
        library = self.client.get("/api/media-library?kind=video").json()
        item = next(entry for entry in library["items"] if entry["filename"] == "clip.mp4")
        ...
        self.assertEqual(payload.get("purgedVps"), 1)
        self.assertNotIn(item["id"], payload.get("statuses") or {})
        self.assertFalse(self.video.exists())
        library_after = self.client.get("/api/media-library?kind=video").json()
        self.assertFalse(
            any(entry["id"] == item["id"] for entry in library_after["items"])
        )

    async def test_mac_snapshot_purges_by_unique_size_when_renamed(self):
        """Motrix date-named Mac copies often lose recordingId and basename match."""
        library = self.client.get("/api/media-library?kind=video").json()
        item = next(entry for entry in library["items"] if entry["filename"] == "clip.mp4")
        self.assertTrue(self.video.exists())
        ...
        snapshot = self.client.post(
            ...
            json={
                ...
                "files": [{
                    "recordingId": None,
                    "filename": "tiffy(Twitch)/2026-09-05 02-30-08.mp4",
                    "size": item["size"],
                }],
            },
        )
    ...
    async def test_mac_download_job_accepts_motrix_method(self):
        library = self.client.get("/api/media-library?kind=video").json()
        item = next(entry for entry in library["items"] if entry["filename"] == "clip.mp4")
        created = self.client.post(
            "/api/mac/download-jobs",
        ...
        self.assertEqual(created.status_code, 200)
        self.assertEqual(created.json()["method"], "motrix")
        job_id = created.json()["jobId"]
        job = self.client.get(
            f"/api/mac/download-jobs/{job_id}?localSessionId=mac-session"
        )
        self.assertEqual(job.status_code, 200)
        self.assertEqual(job.json()["method"], "motrix")
    ...
    async def test_mac_autosync_removed_stays_disabled(self):
        """Auto Sync removed: setting and helper heartbeat always report false."""
        ...
        self.assertFalse(heartbeat.json()["macAutoSync"])
        self.assertFalse(await app_main._mac_autosync_enabled())

    async def test_recording_settings_include_mac_autosync(self):
        response = self.client.get("/api/settings/recording")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertFalse(data["mac_auto_sync"])
        self.assertIn("gap_buffer_minutes", data)
        self.assertEqual(data["gap_buffer_minutes"], 30)
        self.assertEqual(data["download_project_concurrency"], 2)
        self.assertEqual(data["download_file_concurrency"], 6)
        ...
        updated = self.client.put(
            ...
            json={
                ...
                "download_project_concurrency": 3,
                "download_file_concurrency": 8,
            },
        )
        self.assertEqual(updated.status_code, 200)
        self.assertFalse(updated.json()["mac_auto_sync"])
        self.assertEqual(updated.json()["gap_buffer_minutes"], 60)
        self.assertEqual(updated.json()["download_project_concurrency"], 3)
        self.assertEqual(updated.json()["download_file_concurrency"], 8)

    async def test_global_recording_pause_stops_sessions_without_clearing_auto_record(self):
        username = "pause-record-model"
        await app_main.db.add_or_update_model(
            username,
            source_type="chaturbate",
            auto_record=True,
        ...
        self.assertFalse(paused.json()["recording_enabled"])
        self.assertEqual(stopped, [("sess-pause-1", "global_recording_pause")])
        ...
        self.assertIn("paused globally", blocked.json()["detail"])
        ...
        self.assertTrue(resumed.json()["recording_enabled"])
        model_after = await app_main.db.get_model(username, source_type="chaturbate")
        self.assertTrue(model_after["auto_record"])

    async def test_mac_auto_sync_setting_force_false(self):
        """Clients may still POST mac_auto_sync; server always stores/returns false."""
        heartbeat = self.client.post(
            "/api/mac/helper/heartbeat",
            json={
                "localSessionId": "mac-session",
    ...
    async def test_helper_heartbeat_snapshot_and_pending_job_claim(self):
        library = self.client.get("/api/media-library?kind=video").json()
        item = next(entry for entry in library["items"] if entry["filename"] == "clip.mp4")

        empty = self.client.get("/api/mac/helper/snapshot")
        ...
        self.assertEqual(heartbeat.status_code, 200)
        pending = heartbeat.json()["pendingJobs"]
        self.assertEqual(len(pending), 1)
        self.assertEqual(pending[0]["jobId"], job_id)
        ...
        snap = self.client.get("/api/mac/helper/snapshot")
        self.assertEqual(snap.status_code, 200)
        payload = snap.json()
        self.assertTrue(payload["available"])
        self.assertEqual(payload["localSessionId"], "mac-session")
        # Complete Mac match deletes the VPS staging copy.
        self.assertEqual(payload.get("purgedVps"), 1)
        self.assertNotIn(item["id"], payload.get("statuses") or {})
        self.assertEqual(payload["files"][0]["filename"], "clip.mp4")
        self.assertFalse(self.video.exists())

    async def test_helper_open_and_delete_queue_through_vps(self):
        ...
        self.assertEqual(opened.json()["status"], "queued")
        self.assertEqual(opened.json()["type"], "open")
        self.assertTrue(opened.json()["reveal"])

        claimed = self.client.get("/api/mac/helper/commands?localSessionId=mac-session")
        self.assertEqual(claimed.status_code, 200)
        commands = claimed.json()["pendingCommands"]
        self.assertEqual(len(commands), 1)
        self.assertEqual(commands[0]["type"], "open")
        self.assertEqual(commands[0]["relativePath"], "model/clip.mp4")
        self.assertEqual(commands[0]["status"], "claimed")

        empty = self.client.get("/api/mac/helper/commands?localSessionId=mac-session")
        self.assertEqual(empty.json()["pendingCommands"], [])

        deleted = self.client.post(
            "/api/mac/helper/delete",
            json={
                "localSessionId": "mac-session",
                "items": [{"relativePath": "model/clip.mp4", "recordingId": "rec_clip"}],
            },
        )
        self.assertEqual(deleted.status_code, 200)
        self.assertEqual(deleted.json()["status"], "queued")
        ...
        self.assertEqual(heartbeat.status_code, 200)
        pending = heartbeat.json()["pendingCommands"]
        self.assertEqual(len(pending), 1)
        self.assertEqual(pending[0]["type"], "delete")
        self.assertEqual(pending[0]["status"], "claimed")

        wrong = self.client.post(
            "/api/mac/helper/open",
            json={"localSessionId": "other-session", "relativePath": "model/clip.mp4"},
        )
        self.assertEqual(wrong.status_code, 503)

    async def test_helper_thumb_upload_and_browser_get(self):
        import base64
        ...
        fetched = self.client.get(
            "/api/mac/helper/thumb?localSessionId=mac-session&relativePath=model/clip.mp4"
        )
        ...
        missing = self.client.get(
            "/api/mac/helper/thumb?localSessionId=mac-session&relativePath=model/missing.mp4"
        )
        self.assertEqual(missing.status_code, 404)
        queued = self.client.get("/api/mac/helper/commands?localSessionId=mac-session")
        commands = queued.json()["pendingCommands"]
        self.assertEqual(len(commands), 1)
        self.assertEqual(commands[0]["type"], "thumb")
        self.assertEqual(commands[0]["relativePath"], "model/missing.mp4")

    async def test_helper_thumb_post_is_public_get_is_not(self):
        import base64
        ...
        try:
            ...
            uploaded = self.client.post(
                ...
                json={
                    "localSessionId": "mac-session",
                    "relativePath": "model/clip.mp4",
                    "imageBase64": base64.b64encode(b"png").decode("ascii"),
                },
            )
            self.assertEqual(uploaded.status_code, 200)
            fetched = self.client.get(
    ...
    async def test_helper_commands_are_public_when_password_enabled(self):
        original_password = app_main.PASSWORD
        original_sessions = set(app_main.active_sessions)
        app_main.PASSWORD = "secret"
        app_main.active_sessions.clear()
        try:
            ...
            queued = self.client.post(
                "/api/mac/helper/open",
                json={"localSessionId": "mac-session", "relativePath": "model/clip.mp4"},
            )
    ...
    async def test_helper_heartbeat_is_public_when_password_enabled(self):
        original_password = app_main.PASSWORD
        original_sessions = set(app_main.active_sessions)
        app_main.PASSWORD = "secret"
        app_main.active_sessions.clear()
        try:
            ...
            self.assertEqual(heartbeat.status_code, 200)
            snapshot = self.client.get("/api/mac/helper/snapshot")
    ...
    async def test_processing_files_api_lists_and_deletes_temporary_files(self):
        temp_mp4 = self.records_dir / ".clip.abcdef.tmp.mp4"
        temp_mp4.write_bytes(b"temporary")

        listing = self.client.get("/api/media-library/processing-files")
        self.assertEqual(listing.status_code, 200)
        payload = listing.json()
        paths = {item["path"] for item in payload["files"]}
        self.assertIn("model/raw.ts", paths)
        self.assertIn("model/.clip.abcdef.tmp.mp4", paths)
        self.assertNotIn("model/clip.mp4", paths)
        self.assertIn(str(self.output_dir / "records"), payload["recordsRoot"])

        delete = self.client.post(
            "/api/media-library/processing-files/delete",
            json={"paths": ["model/raw.ts", "model/.clip.abcdef.tmp.mp4"]},
        )
        self.assertEqual(delete.status_code, 200)
        self.assertEqual(delete.json()["deletedCount"], 2)
        self.assertFalse(self.ts_file.exists())
        self.assertFalse(temp_mp4.exists())

        empty = self.client.get("/api/media-library/processing-files")
        self.assertEqual(empty.status_code, 200)
        self.assertEqual(empty.json()["total"], 0)

    async def test_processing_files_delete_rejects_non_temporary_files(self):
        response = self.client.post(
            "/api/media-library/processing-files/delete",
            json={"paths": ["model/clip.mp4"]},
        )
        self.assertEqual(response.status_code, 400)
    ...
    async def test_profile_recording_can_enable_without_follow_and_can_be_paused(self):
        create = self.client.put(
            "/api/media-profiles/empty_model",
            json={
    ...
    async def test_auto_record_bootstraps_missing_profile_with_source_hint(self):
        """Mac-local cards can enable recording before a VPS profile row exists."""
        missing = self.client.get("/api/media-profiles/mac_local_tiffy")
        self.assertEqual(missing.status_code, 404)

        enabled = self.client.patch(
            "/api/media-profiles/mac_local_tiffy/auto-record",
            json={
                "autoRecord": True,
                "sourceType": "twitch",
                "channelUsername": "mac_local_tiffy",
                "channelUrl": "https://www.twitch.tv/mac_local_tiffy",
                "displayName": "Mac Local Tiffy",
            },
        )
        self.assertEqual(enabled.status_code, 200, enabled.text)
        self.assertTrue(enabled.json()["autoRecord"])
        profile = await app_main.db.get_media_profile("mac_local_tiffy")
        self.assertIsNotNone(profile)
        sources = await app_main.db.get_media_profile_sources("mac_local_tiffy")
        self.assertEqual(len(sources), 1)
        self.assertEqual(sources[0]["source_type"], "twitch")
        self.assertTrue(sources[0]["auto_record"])
        model = await app_main.db.get_model("mac_local_tiffy", source_type="twitch")
        self.assertIsNotNone(model)
        self.assertTrue(model["auto_record"])

        bulk = self.client.post(
            "/api/media-profiles/bulk/auto-record",
            json={
                "autoRecord": True,
                "items": [{
                    "username": "bulk_mac_local",
                    "sourceType": "twitch",
                    "channelUrl": "https://www.twitch.tv/bulk_mac_local",
                    "displayName": "Bulk Mac",
                }],
            },
        )
        self.assertEqual(bulk.status_code, 200, bulk.text)
        self.assertEqual(bulk.json()["updated"], 1)
        self.assertTrue(bulk.json()["results"][0]["success"])

    async def test_auto_record_prefers_body_source_over_existing_wrong_source(self):
        """Discover Twitch Record must not stick to a leftover chaturbate source row."""
        await app_main.db.upsert_media_profile("yawn", {"display_name": "Yawn"})
        await app_main.db.upsert_media_profile_source(
            profile_username="yawn",
            source_type="chaturbate",
            channel_username="yawn",
            channel_url="https://chaturbate.com/yawn/",
            auto_record=False,
            record_quality="best",
            retention_days=30,
            record_path=None,
        ...
        enabled = self.client.patch(
            "/api/media-profiles/yawn/auto-record",
            json={
                "autoRecord": True,
                "sourceType": "twitch",
                "channelUsername": "yawn",
                "channelUrl": "https://www.twitch.tv/yawn",
                "displayName": "Yawn",
            },
        )
        self.assertEqual(enabled.status_code, 200, enabled.text)
        self.assertTrue(enabled.json()["autoRecord"])
        sources = await app_main.db.get_media_profile_sources("yawn")
        by_type = {row["source_type"]: row for row in sources}
        self.assertIn("twitch", by_type)
        self.assertTrue(by_type["twitch"]["auto_record"])
        self.assertIn("chaturbate", by_type)
        self.assertFalse(by_type["chaturbate"]["auto_record"])
        model = await app_main.db.get_model("yawn", source_type="twitch")
        self.assertIsNotNone(model)
        self.assertTrue(model["auto_record"])
        cb_model = await app_main.db.get_model("yawn", source_type="chaturbate")
        if cb_model:
            self.assertFalse(cb_model["auto_record"])

        library = self.client.get("/api/media-library?limit=1&metadata=lazy")
        self.assertEqual(library.status_code, 200)
        profiles = library.json().get("profiles") or []
        yawn = next((p for p in profiles if p.get("username") == "yawn"), None)
        self.assertIsNotNone(yawn)
        self.assertEqual(yawn.get("sourceType") or yawn.get("source_type"), "twitch")
        self.assertTrue(yawn.get("autoRecord"))
        stream_sources = yawn.get("streamSources") or []
        twitch_src = next(
            (s for s in stream_sources if (s.get("sourceType") or s.get("source_type")) == "twitch"),
            None,
        )
        self.assertIsNotNone(twitch_src)
        self.assertTrue(twitch_src.get("autoRecord") or twitch_src.get("auto_record"))

    async def test_link_live_rejects_create_profile(self):
        response = self.client.post(
            "/api/media-profiles/link-live",
            json={
                "createProfile": True,
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn("Follow the streamer first", response.json().get("detail") or "")
    ...
    def test_media_live_profile_image_skips_twitch_stream_preview(self):
        """Twitch previews-ttv frames must never become circular avatars."""
        face = "https://static-cdn.jtvnw.net/jtv_user_pictures/tiffy-profile_image-300x300.png"
        live_preview = (
            "https://static-cdn.jtvnw.net/previews-ttv/live_user_tiffy-440x248.jpg"
        )
        self.assertTrue(app_main._is_live_cover_profile_image(live_preview))
        self.assertFalse(app_main._is_live_cover_profile_image(face))
        self.assertEqual(
            face,
            app_main._media_live_profile_image_url(
                {"profile_image_url": face, "thumbnail": live_preview},
                source_type="twitch",
                is_online=True,
            ),
        )
        self.assertEqual(
            "",
            app_main._media_live_profile_image_url(
                {"profile_image_url": live_preview, "thumbnail": live_preview},
                source_type="twitch",
                is_online=True,
            ),
        )

    async def test_model_status_prefers_live_face_over_babepedia(self):
        """Babepedia can resolve the wrong person (Lillyfee for Tiffy)."""
        await app_main.db.upsert_media_profile(
            "tiffy",
            {
                "display_name": "Tiffy",
                "profile_image_url": "https://www.babepedia.com/pics/Lillyfee_thumb3.jpg",
                "profile_image_source_url": "https://www.babepedia.com/babe/Tiffy",
            },
        )
        helix_face = (
            "https://static-cdn.jtvnw.net/jtv_user_pictures/tiffy-profile_image-300x300.png"
        )

        async def fake_live_meta(source_type, username, force_refresh=False):
            self.assertEqual("twitch", source_type)
            return {
                "isOnline": False,
                "viewers": 0,
                "followers": 109327,
                "profileImageUrl": helix_face,
                "displayName": "Tiffy",
                "channelUrl": "https://www.twitch.tv/tiffy",
            }

        with patch.object(app_main, "_media_profile_live_card_meta", side_effect=fake_live_meta), \
             patch.object(app_main, "_available_source_types", return_value={"twitch"}):
            response = self.client.get("/api/model/tiffy/status?source=twitch")

        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(helix_face, data.get("profileImageUrl"))
        self.assertNotIn("babepedia", str(data.get("profileImageUrl") or "").lower())
        self.assertNotIn("Lillyfee", str(data.get("profileImageUrl") or ""))

    def test_pick_media_card_avatar_prefers_live_over_babepedia(self):
        babepedia = {
            "username": "tiffy",
            "profile_image_url": "https://www.babepedia.com/pics/Lillyfee_thumb3.jpg",
            "profile_image_source_url": "https://www.babepedia.com/babe/Tiffy",
        }
        helix = "https://static-cdn.jtvnw.net/jtv_user_pictures/tiffy-profile_image-300x300.png"
        self.assertEqual(
            helix,
            app_main._pick_media_card_avatar(
                media_profile=babepedia,
                live_image=helix,
            ),
        )

    async def test_model_status_does_not_promote_twitch_live_preview_as_avatar(self):
        """Local Media cards hydrate via /status — stream thumbs must stay covers."""
        from app.providers.base import ProviderStatus

        async def fake_live_meta(source_type, username, force_refresh=False):
            return {}

        async def fake_provider_status(source_type, username):
            return ProviderStatus(
                ...
                thumbnail=(
                    ...
                    "live_user_tiffy-440x248.jpg"
                ),
                source_type="twitch",
            )

        with patch.object(app_main, "_media_profile_live_card_meta", side_effect=fake_live_meta), \
             patch.object(app_main, "_provider_status", side_effect=fake_provider_status), \
             patch.object(app_main, "_available_source_types", return_value={"twitch"}):
            response = self.client.get("/api/model/tiffy/status?source=twitch")

        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data.get("isOnline"))
        self.assertEqual("", data.get("profileImageUrl") or "")
        self.assertIn("previews-ttv", data.get("thumbnail") or "")
    ...
    async def test_delete_media_profile_removes_followed_so_card_does_not_ghost(self):
        await app_main.db.upsert_media_profile(
            "obviouslasmr",
            {"display_name": "Obvious"},
        )
        await app_main.db.upsert_media_profile_source(
            profile_username="obviouslasmr",
            source_type="twitch",
            channel_username="obviouslasmr",
            channel_url="https://www.twitch.tv/obviouslasmr",
            auto_record=False,
            record_quality="best",
            retention_days=30,
            record_path=None,
        ...
        delete = self.client.delete("/api/media-profiles/obviouslasmr")
        self.assertEqual(delete.status_code, 200)
        self.assertIsNone(await app_main.db.get_media_profile("obviouslasmr"))
        self.assertIsNone(await app_main.db.get_followed_model("obviouslasmr", "twitch"))

        listing = self.client.get("/api/media-library")
        self.assertEqual(listing.status_code, 200)
        profiles = {item["username"]: item for item in listing.json()["profiles"]}
        self.assertNotIn("obviouslasmr", profiles)

        ghost_delete = self.client.delete("/api/media-profiles/obviouslasmr")
        self.assertEqual(ghost_delete.status_code, 404)

    async def test_delete_followed_only_does_not_create_media_card(self):
        await app_main.db.upsert_followed_model(
            "ghost_only",
            display_name="Ghost",
            source_type="twitch",
            profile_image_url="https://static-cdn.jtvnw.net/jtv_user_pictures/ghost.png",
            followers=42,
        )
        ...
        self.assertEqual(listing.status_code, 200)
        profiles = {item["username"]: item for item in listing.json()["profiles"]}
        self.assertNotIn("ghost_only", profiles)

        delete = self.client.delete("/api/media-profiles/ghost_only")
        self.assertEqual(delete.status_code, 200)
        self.assertIsNone(await app_main.db.get_followed_model("ghost_only", "twitch"))
