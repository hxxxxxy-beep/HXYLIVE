...
class DiscoverCategoriesApiTests(unittest.IsolatedAsyncioTestCase):
    async def test_youtube_categories_endpoint(self):
        payload = await discover.discover_categories(source="youtube")
        ...
        self.assertTrue(labels.isdisjoint({"Female", "Male", "Trans", "Couple", "All", "ASMR"}))
    ...
    async def test_bilibili_categories_endpoint(self):
        ...
        self.assertEqual("Virtual streamers", area["display_label"])
        labels = {c["display_label"] for c in payload["categories"]}
        self.assertTrue(labels.isdisjoint({"Female", "Male", "Trans", "Couple", "All", "Radio"}))
    ...
    async def test_supported_sources_still_listed_via_catalog(self):
        with patch.object(
            ...
            for source in ("twitch", "chaturbate", "bilibili", "youtube"):
