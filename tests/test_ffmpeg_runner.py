...
class FFmpegCommandTests(unittest.TestCase):
    ...
    def test_proxied_chaturbate_llhls_uses_source_url_for_mapping(self):
        ...
        # Chaturbate HTTP knobs follow source_url even when -i is loopback.
        self.assertEqual(cmd[cmd.index("-http_multiple") + 1], "0")

    def test_filesystem_playlist_skips_reconnect_and_outbound_proxy(self):
        with (
            patch("app.ffmpeg_runner.ffmpeg_http_proxy_url", return_value="http://127.0.0.1:7897"),
            patch("app.ffmpeg_runner.get_outbound_proxy_url", return_value="http://127.0.0.1:7897"),
            patch("app.ffmpeg_runner.is_socks_proxy", return_value=False),
        ):
            cmd = _build_ffmpeg_command(
                "ffmpeg",
                "/data/sessions/_ffmpeg_playlists/master.m3u8",
                "tee-output",
                source_url="https://edge30-ash.live.mmcdn.com/live/test/llhls.m3u8",
                input_headers={"Referer": "https://chaturbate.com/"},
        ...
        self.assertEqual(
            cmd[cmd.index("-protocol_whitelist") + 1],
            "file,http,https,tcp,tls,crypto",
        )
        self.assertLess(cmd.index("-protocol_whitelist"), cmd.index("-i"))
        self.assertEqual(cmd[cmd.index("-http_multiple") + 1], "0")
        self.assertEqual(["0:v:1", "0:a:0"], self._maps(cmd))

    def test_local_hls_proxy_keeps_user_agent_and_whitelist(self):
        with (
            patch("app.ffmpeg_runner.ffmpeg_http_proxy_url", return_value="http://127.0.0.1:7897"),
            patch("app.ffmpeg_runner.get_outbound_proxy_url", return_value="http://127.0.0.1:7897"),
            patch("app.ffmpeg_runner.is_socks_proxy", return_value=False),
...
class FFmpegLockDeadlockTests(unittest.TestCase):
    def test_busy_snapshot_while_holding_lock_does_not_deadlock(self):
        """Regression: refresh_busy_snapshot used to call list_status() while
        start/stop already held self._lock, freezing the whole site."""
        import threading
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def worker():
                try:
                    with manager._lock:
                        recording_audit.refresh_busy_snapshot(manager._statuses_locked())
            ...
            thread.start()
            self.assertTrue(done.wait(timeout=2.0), "deadlocked on manager lock")
