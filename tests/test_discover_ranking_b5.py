...
class DiscoverFrontendContractTests(unittest.TestCase):
    ...
    def test_ui_font_is_cjk_serif_stack_including_search_inputs(self):
        # Regression: search/tag inputs must not be forced to Inter / system sans.
        # No bundled OTF — system CJK serif stack only (keeps the repo lightweight).
        self.assertNotIn("SourceHanSerifCN-Medium.otf", self.css)
        self.assertNotIn("@font-face", self.css)
        self.assertIn("--font-ui:", self.css)
        self.assertIn("Noto Serif CJK SC", self.css)
        self.assertIn("Songti SC", self.css)
        self.assertIn(".search-input", self.css)
        self.assertIn(".tag-input", self.css)
        self.assertRegex(
            self.css,
            r"\.search-input\s*\{[^}]*font-family:\s*var\(--font-ui\)\s*!important",
        )
        self.assertRegex(
            self.css,
            r"\.tag-input\s*\{[^}]*font-family:\s*var\(--font-ui\)\s*!important",
        )
        self.assertNotIn("fonts.googleapis.com", self.html)
        self.assertNotIn("family=Inter", self.html)
        self.assertNotIn("font-family: Inter", self.css)
