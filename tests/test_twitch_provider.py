...
class TwitchProviderTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_resolve_watch_meta_uses_exact_helix_user_not_search(self):
        """Media local cards must not miss short logins like tiffy via search pages."""
        provider = _provider()
        face = "https://static-cdn.jtvnw.net/jtv_user_pictures/tiffy-profile_image-300x300.png"
        live_preview = (
            "https://static-cdn.jtvnw.net/previews-ttv/live_user_tiffy-{width}x{height}.jpg"
        )
        with patch.object(
            provider,
            "_helix_users",
            ...
            provider,
            "_helix_streams_by_user_ids",
            AsyncMock(return_value={
                "99": _stream("99", "tiffy", viewers=12, thumbnail_url=live_preview),
            }),
        ), patch.object(
            provider,
            "_follower_total",
            ...
            meta = await provider.resolve_watch_meta("tiffy")

        self.assertEqual("tiffy", meta["username"])
        self.assertEqual("Tiffy", meta["displayName"])
        self.assertTrue(meta["isOnline"])
        self.assertEqual(12, meta["viewers"])
        self.assertEqual(1234, meta["followers"])
        self.assertEqual(face, meta["profileImageUrl"])
        self.assertIn("previews-ttv", meta["thumbnail"])
        self.assertNotIn("previews-ttv", meta["profileImageUrl"])
...
class TwitchFollowingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_verify_logged_in_false_without_auth_token(self):
        provider = self._provider()
        self.assertFalse(await provider.verify_logged_in())

    async def test_verify_logged_in_true_when_current_user_resolves(self):
        provider = self._provider()
        ...
        with patch.object(
            ...
            self.assertTrue(await provider.verify_logged_in())

    async def test_verify_logged_in_false_when_current_user_auth_fails(self):
        ...
        with patch.object(
            provider,
            "_twitch_current_user",
            AsyncMock(side_effect=ProviderAuthError("Twitch login required")),
        ):
            self.assertFalse(await provider.verify_logged_in())
    ...
    async def test_sync_following_falls_back_to_gql_when_helix_is_empty(self):
        provider = self._provider()
        await provider.session_store.save(
            "twitch",
            username="alice",
                ...
                "follows": {
                    "edges": [{
                        "node": {
                            ...
                            "profileImageURL": "https://static-cdn.example/streamer.png",
                            "stream": None,
                        }
                    }],
        ...
        with patch.object(
            ...
            provider,
            "_user_helix_get",
            AsyncMock(return_value={"data": [], "pagination": {}}),
        ), patch.object(
            provider,
            "_twitch_gql",
            gql,
            ...
            items = await provider.sync_following()
        self.assertEqual(["streamer"], [item["username"] for item in items])
        gql.assert_awaited()

    async def test_sync_following_falls_back_to_gql_when_helix_auth_fails(self):
        from app.providers.base import ProviderAuthError
    ...
    async def test_follow_uses_gql_mutation(self):
        ...
        with patch.object(
            ...
            provider,
            "_twitch_integrity_token",
            AsyncMock(return_value="integrity-token"),
        ...
        self.assertTrue(gql.await_args.kwargs.get("require_integrity"))

    async def test_twitch_user_id_falls_back_to_gql(self):
        provider = self._provider()
        await provider.session_store.save(
            "twitch",
            username="alice",
        ...
        gql = AsyncMock(return_value={"user": {"id": "42", "login": "streamer"}})
        with patch.object(
            provider,
            "_helix_users",
            AsyncMock(side_effect=RuntimeError("no app credentials")),
        ), patch.object(provider, "_twitch_gql", gql):
            user_id = await provider._twitch_user_id("streamer")
        self.assertEqual("42", user_id)
        self.assertIn("HXYLIVEUserId", gql.await_args.args[0])

    async def test_sync_following_prefers_gql_when_helix_faces_missing(self):
        provider = self._provider()
        await provider.session_store.save(
            "twitch",
            username="alice",
        ...
        gql_items = [{
            ...
            "viewers": 0,
            "source_type": "twitch",
            "profile_image_url": "https://static-cdn.jtvnw.net/jtv_user_pictures/face.png",
        }]
        with patch.object(
            provider,
            "_twitch_current_user",
            ...
            provider,
            "_twitch_sync_helix_followed",
            AsyncMock(return_value=helix_items),
        ), patch.object(
            provider,
            "_twitch_sync_gql_followed",
            AsyncMock(return_value=gql_items),
        ), patch.object(
            provider,
            "_hydrate_followed_live_status",
            AsyncMock(side_effect=lambda items: items),
            ...
            items = await provider.sync_following()
        self.assertEqual(
            "https://static-cdn.jtvnw.net/jtv_user_pictures/face.png",
            items[0]["profile_image_url"],
        )
