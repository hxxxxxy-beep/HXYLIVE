...
class StripchatProviderC1Tests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_model_by_username_falls_back_to_users_when_cam_418(self):
        """Offline cams often HTTP 418; /users + /models/{id} resolve profile media."""
        provider = _provider()
        ...
        async def _api(method, path, params=None, **kwargs):
            if path.endswith("/cam"):
                ...
                raise ProviderError("Stripchat API HTTP 418")
            ...
            if path == "/models/184041870":
                return {
                    ...
                    "isOnline": False,
                    "isLive": False,
                    "avatarUrl": "https://static-proxy.strpst.com/avatars/f/3/c/ruby-full",
                }
            raise AssertionError(f"unexpected path {path}")

        with patch.object(provider, "_stripchat_api_json", AsyncMock(side_effect=_api)):
            model = await provider._stripchat_model_by_username("Ruby_saki")
        self.assertEqual("Ruby_saki", model["username"])
        self.assertFalse(model["isOnline"])
        self.assertEqual(
            "https://static-proxy.strpst.com/avatars/f/3/c/ruby-full",
            model["avatarUrl"],
        )

    async def test_exact_search_returns_offline_user_when_cam_unavailable(self):
        provider = _provider()
        ...
        async def _api(method, path, params=None, **kwargs):
            ...
            # Catalogue fallback should not be required for exact offline hit.
        ...
        with patch.object(provider, "_stripchat_api_json", AsyncMock(side_effect=_api)):
            with patch.object(
                ...
                payload = await provider.list_live_models(
                    page=1, limit=12, search="Ruby_saki"
                )
        self.assertEqual(1, len(payload["models"]))
        item = payload["models"][0]
        self.assertEqual("Ruby_saki", item["username"])
        self.assertFalse(item["is_online"])
        self.assertEqual("offline", item["room_status"])
        self.assertEqual(
            "https://static-proxy.strpst.com/avatars/f/3/c/ruby-full",
...
class StripchatSessionTests(unittest.IsolatedAsyncioTestCase):
    def _provider(self):
        provider = _provider()
        provider.session_store = _MemorySessionStore()
        return provider

    async def test_import_session_requires_session_id(self):
        provider = self._provider()
        result = await provider.import_session(cookie_header="guestID=guest")
        self.assertFalse(result["success"])
        self.assertIn("sessionId", result["error"])

    async def test_import_session_rejects_guest_payload(self):
        provider = self._provider()
        with patch.object(
            provider,
            "_stripchat_api_json",
            AsyncMock(return_value={"currentUser": {"id": 1, "isGuest": True}}),
        ):
            result = await provider.import_session(cookie_header="sessionId=sess")
        self.assertFalse(result["success"])
        saved = await provider.session_store.get("stripchat")
        self.assertFalse(saved["is_logged_in"])

    async def test_import_session_accepts_authenticated_user(self):
        provider = self._provider()
        with patch.object(
            provider,
            "_stripchat_api_json",
            AsyncMock(
                return_value={
                    "currentUser": {"id": 9, "username": "alice", "isGuest": False},
                    "jwtToken": "jwt",
                }
            ),
        ):
    ...
    async def test_import_session_accepts_prefixed_session_cookie(self):
        provider = self._provider()
        with patch.object(
            provider,
            "_stripchat_api_json",
            ...
            result = await provider.import_session(
                cookie_header="stripchat_com_sessionId=sess-new"
            )
        self.assertTrue(result["success"])
        saved = await provider.session_store.get("stripchat")
        names = {item.get("name") for item in (saved.get("cookies") or [])}
        self.assertIn("stripchat_com_sessionId", names)
        self.assertIn("sessionId", names)

    async def test_verify_logged_in_false_without_session_id(self):
        provider = self._provider()
        await provider.session_store.save(
            "stripchat",
            is_logged_in=True,
            cookies=[{"name": "guestID", "value": "guest"}],
        )
        self.assertFalse(await provider.verify_logged_in())

    async def test_has_session_uses_cookies_not_logged_in_flag(self):
        provider = self._provider()
        await provider.session_store.save(
            "stripchat",
            username="alice",
            is_logged_in=False,
            cookies=[{"name": "sessionId", "value": "sess"}],
        )
        self.assertTrue(await provider._stripchat_has_session())

    async def test_api_json_retries_favorites_without_stale_jwt(self):
        provider = self._provider()
        await provider.session_store.save(
            "stripchat",
            username="alice",
            ...
            local_storage=[{
                ...
                "localStorage": [{"name": "jwtToken", "value": "stale-jwt"}],
            }],
        )
        ...
        class _Session:
            ...
            def request(self, method, url, **kwargs):
                ...
                self.calls.append({"url": url, "Authorization": headers.get("Authorization")})
                if "Authorization" in headers:
                    return _Resp(401, '{"title":"An error occurred","description":"invalid token"}')
                return _Resp(200, '{"models":[{"id":1,"username":"fav1","isOnline":true}],"totalCount":1}')
        ...
        with patch("app.providers.stripchat.aiohttp_client_session", return_value=fake_session):
            payload = await provider._stripchat_api_json(
                "GET",
                "/models/favorites",
                params={"limit": 50, "offset": 0},
                auth_required=True,
                referer="https://stripchat.com/favorites",
            )
        self.assertEqual(1, payload["totalCount"])
        self.assertEqual(2, len(fake_session.calls))
        self.assertEqual("stale-jwt", fake_session.calls[0]["Authorization"])
        self.assertIsNone(fake_session.calls[1]["Authorization"])
        saved = await provider.session_store.get("stripchat")
        jwt = provider._stripchat_find_value(saved.get("localStorage"), {"jwttoken", "jwt"})
        self.assertFalse(jwt)

    async def test_sync_following_persists_refreshed_jwt(self):
        provider = self._provider()
        await provider.session_store.save(
            "stripchat",
            username="alice",
            ...
            local_storage=[{
                "origin": "https://stripchat.com",
                "localStorage": [{"name": "jwtToken", "value": "old"}],
            }],
        )

        async def _api(method, path, **kwargs):
            if "config" in path or path.endswith("/users/current"):
                return {
                    "currentUser": {"id": 9, "username": "alice", "isGuest": False},
                    "jwtToken": "fresh-jwt",
                }
            return {
                "models": [{"id": 1, "username": "fav1", "status": "public", "isOnline": True}],
                "totalCount": 1,
            }
        ...
        saved = await provider.session_store.get("stripchat")
        jwt = provider._stripchat_find_value(saved.get("localStorage"), {"jwttoken", "jwt"})
        self.assertEqual("fresh-jwt", jwt)
