...
(function (root) {
  ...
  // /api/discover accepts gender (+ implicit all), Twitch game_id, Bilibili parent_area_id.
  // YouTube Discover is exact @handle search only (no category request params).
  var SUPPORTED_DISCOVER_REQUEST_PARAMS = {
    gender: true,
    game_id: true,
    parent_area_id: true,
    video_category_id: true,
    live_query: true
  };
  ...
  function normalizeCategoryItem(item) {
    ...
    if (requestParam === undefined || requestValue === undefined) {
      ...
      if (ctype === 'all' || key === 'all') {
        ...
        } else if (String(key).indexOf('parent_area:') === 0) {
          requestParam = 'parent_area_id';
          requestValue = String(key).slice(12);
        } else if (String(key).indexOf('video_category:') === 0) {
          requestParam = 'video_category_id';
          requestValue = String(key).slice(15);
        } else if (String(key).indexOf('live_query:') === 0) {
          requestParam = 'live_query';
          requestValue = String(key).slice(11);
        } else {
  ...
  function evaluateCategoryRequestSupport(item) {
    ...
    if (ctype === 'all' || (!param && (normalized.request_value == null || normalized.request_value === ''))) {
      return {
        ...
        discoverVideoCategoryId: null,
        discoverLiveQuery: null
      };
    }
    ...
    if (
      (ctype === 'content' || param === 'video_category_id') &&
      param === 'video_category_id' &&
      normalized.request_value &&
      /^\d+$/.test(String(normalized.request_value))
    ) {
      return {
        supported: true,
        reason: 'video_category_id',
        message: '',
        normalized: normalized,
        discoverGender: null,
        discoverGameId: null,
        discoverParentAreaId: null,
        discoverVideoCategoryId: String(normalized.request_value),
        discoverLiveQuery: null
      };
    }

    if (
      (ctype === 'content' || param === 'live_query') &&
      param === 'live_query' &&
      normalized.request_value &&
      /^[a-z][a-z0-9]{0,31}$/.test(String(normalized.request_value))
    ) {
      return {
        supported: true,
        reason: 'live_query',
        ...
        discoverParentAreaId: null,
        discoverVideoCategoryId: null,
        discoverLiveQuery: String(normalized.request_value).toLowerCase()
      };
    }
  ...
  function applyCategoryRequest(selection) {
    ...
    if (gate.discoverGender) {
      return { ok: true, gender: gate.discoverGender, game_id: '', parent_area_id: '', video_category_id: '', live_query: '' };
    }
    if (gate.discoverGameId) {
      return { ok: true, gender: '', game_id: gate.discoverGameId, parent_area_id: '', video_category_id: '', live_query: '' };
    }
    if (gate.discoverParentAreaId) {
      return { ok: true, gender: '', game_id: '', parent_area_id: gate.discoverParentAreaId, video_category_id: '', live_query: '' };
    }
    if (gate.discoverVideoCategoryId) {
      return { ok: true, gender: '', game_id: '', parent_area_id: '', video_category_id: gate.discoverVideoCategoryId, live_query: '' };
    }
    if (gate.discoverLiveQuery) {
      return { ok: true, gender: '', game_id: '', parent_area_id: '', video_category_id: '', live_query: gate.discoverLiveQuery };
    }
    return { ok: true, gender: '', game_id: '', parent_area_id: '', video_category_id: '', live_query: '' };
  }
  ...
  function preferredDefaultForSource(source) {
    ...
    // YouTube: no category pills — keep selection at All (row stays hidden).
  ...
  function selectDefaultCategory(formalItems, preferred) {
    ...
    if (!candidate) {
      var fallbackItems = safeFallbackItemsForSource(preferredSource);
      if (!fallbackItems.length) return null;
      return normalizeCategoryItem(fallbackItems[0]);
    }
    ...
    if (!gate.supported) {
      return findFormalByKey(items, 'all')
        ...
        || (function () {
          var fb = safeFallbackItemsForSource(preferredSource);
          return fb.length ? normalizeCategoryItem(fb[0]) : null;
        })();
    }
