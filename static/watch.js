...
let hasStreamerCard = false;
let isRecording = false;
let recordingProfileUsername = '';
...
function updateStreamUptimeDisplay() {
  ...
  var text = (source === 'twitch' || source === 'youtube') ? formatStreamUptime(streamStartedAt) : '';
  if (!text) {
...
function setStreamUptime(startedAt, isLive) {
  ...
  stopStreamUptimeTicker();
  var uptimeSource = (currentSourceType || '').toLowerCase();
  if (streamStartedAt && (uptimeSource === 'twitch' || uptimeSource === 'youtube') && isLive) {
...
// ============================================
// Create card / recording (auto-record)
// ============================================
function watchChannelUrl() {
  if (lastWatchChannelUrl) return lastWatchChannelUrl;
  var source = (currentSourceType || 'chaturbate').toLowerCase();
  var username = encodeURIComponent(currentUsername || '');
  var rawName = String(currentUsername || '').trim();
  if (source === 'youtube') {
  ...
  if (source === 'twitch') return 'https://www.twitch.tv/' + username;
  if (source === 'bilibili') return 'https://live.bilibili.com/' + username;
  if (source === 'stripchat') return 'https://stripchat.com/' + username;
  return 'https://chaturbate.com/' + username + '/';
}

function updateWatchActionButtons() {
...
async function loadCardAndRecordingStatus() {
  try {
    ...
    for (var i = 0; i < profiles.length; i++) {
      ...
      for (var j = 0; j < sources.length; j++) {
        var sourceType = String(sources[j].sourceType || sources[j].source_type || 'chaturbate').toLowerCase();
        var channelUsername = String(sources[j].channelUsername || sources[j].channel_username || '').toLowerCase();
        if (channelUsername === String(currentUsername || '').toLowerCase() && sourceType === currentSource) {
          found = profiles[i];
          var sourceRecording = sources[j].autoRecord != null
            ? !!sources[j].autoRecord
            : !!sources[j].auto_record;
          recording = !!(found.autoRecord || sourceRecording);
          break;
        }
      }
      if (found) break;
      if (!sources.length && String(profiles[i].username || '').toLowerCase() === String(currentUsername || '').toLowerCase()) {
        found = profiles[i];
        recording = !!found.autoRecord;
        break;
      }
    }

    hasStreamerCard = !!found;
    recordingProfileUsername = found ? (found.username || currentUsername) : currentUsername;
    isRecording = recording;
    updateWatchActionButtons();
  } catch (e) {
    console.error('Error loading card/recording status:', e);
  }
}
...
function closeWatchCreateCardDialog() {
  var modal = document.getElementById('watchCreateCardModal');
  if (modal) {
...
function openWatchCreateCardDialog() {
  var modal = document.getElementById('watchCreateCardModal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'watchCreateCardModal';
    modal.className = 'hxylive-confirm-modal';
    modal.innerHTML =
      '<div class="hxylive-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="watchCreateCardTitle">' +
        '<h2 id="watchCreateCardTitle">Create card</h2>' +
        '<p id="watchCreateCardHint">Create a local streamer card. Recording is optional and independent.</p>' +
        '<div class="hxylive-confirm-actions discover-create-card-actions">' +
          '<button type="button" class="hxylive-confirm-ok" id="watchCreateCardRecord">Create and record</button>' +
          '<button type="button" class="hxylive-confirm-ok" id="watchCreateCardOnly">Create card only</button>' +
          '<button type="button" class="hxylive-confirm-cancel" id="watchCreateCardCancel">Cancel</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(modal);
    modal.addEventListener('click', function(ev) {
      if (ev.target === modal) closeWatchCreateCardDialog();
    });
    document.getElementById('watchCreateCardCancel').addEventListener('click', closeWatchCreateCardDialog);
    document.getElementById('watchCreateCardOnly').addEventListener('click', function() {
      confirmWatchCreateCard(false);
    });
    document.getElementById('watchCreateCardRecord').addEventListener('click', function() {
      confirmWatchCreateCard(true);
    });
  }
  var title = document.getElementById('watchCreateCardTitle');
  if (title) title.textContent = 'Create card for ' + (lastWatchDisplayName || currentUsername);
  modal.style.display = 'flex';
  modal.setAttribute('aria-hidden', 'false');
}

async function confirmWatchCreateCard(withRecording) {
  closeWatchCreateCardDialog();
  ...
  try {
    var res = await fetch('/api/media-profiles/ensure-card', {
      ...
      body: JSON.stringify({
        ...
        channelUrl: watchChannelUrl(),
        displayName: lastWatchDisplayName || currentUsername,
        autoRecord: !!withRecording
      })
    });
    var data = await res.json().catch(function() { return {}; });
    if (!res.ok) throw new Error(data.detail || 'Failed to create card');
    hasStreamerCard = true;
    recordingProfileUsername = currentUsername;
    isRecording = !!withRecording;
    updateWatchActionButtons();
    showNotification(
      withRecording ? 'Card created and recording enabled' : 'Card created',
      'success'
    );
  } catch (e) {
...
async function toggleRecording() {
  ...
  btn.disabled = true;
  var enabled = !isRecording;
  var profileUsername = recordingProfileUsername || currentUsername;
  var source = currentSourceType || 'chaturbate';
  ...
  try {
    ...
    if (res.status === 404 && enabled) {
      await confirmWatchCreateCard(true);
      return;
    }
    if (res.ok) {
      isRecording = enabled;
      updateRecordButton();
      showNotification(
        enabled ? 'Recording enabled for ' + currentUsername : 'Recording paused for ' + currentUsername,
        'success'
      );
    } else {
