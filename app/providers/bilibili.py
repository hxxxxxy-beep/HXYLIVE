...
class BilibiliProvider(YtDlpProvider):
    ...
    async def list_live_models(self, page: int = 1, limit: int = 24, search: str = "", **kwargs):
        ...
        # tag to getRoomList parent_area_id+area_id, but only inside the curated
        # Virtual streamers allowlist.
        tag_needles = [
            str(tag).strip()
        ...
        if tag_needles and not normalize_bilibili_area_id(area_id):
            ...
            if resolved and is_curated_bilibili_parent_area_id(resolved.get("parent_area_id")):
    ...
    async def sync_following(self) -> list[dict[str, object]]:
        ...
        live_items = await self._bili_sync_live_following()
        if live_items:
            live_by = {
            ...
            for item in items:
                live = live_by.get(str(item.get("username") or ""))
                if not live:
                    continue
                item["is_online"] = bool(live.get("is_online"))
                item["viewers"] = int(live.get("viewers") or 0) if live.get("is_online") else 0
                item["room_status"] = live.get("room_status") or item.get("room_status")
                if live.get("display_name"):
                    item["display_name"] = live["display_name"]
                if live.get("profile_image_url"):
                    item["profile_image_url"] = live["profile_image_url"]
                    item["thumbnail_url"] = live.get("thumbnail_url") or item.get("thumbnail_url")
        items.sort(
            key=lambda item: (
    ...
    def _bili_follow_item_from_live_row(self, row: dict[str, Any]) -> Optional[dict[str, object]]:
        ...
        face = self._absolute_url(row.get("face") or "")
        ...
        return {
            ...
            "thumbnail_url": face or self._absolute_url(row.get("user_cover") or ""),
            "profile_image_url": face,
            "source_type": "bilibili",
