...
class TwitchProvider(YtDlpProvider):
    ...
    async def resolve_watch_meta(self, username: str) -> dict[str, Any]:
        """Exact Helix login lookup for Media/Watch cards (avatar, live, followers).

        Discover channel search is partition-scoped and page-sliced; short
        logins like ``tiffy`` often sit past the first page. Never use keyword
        search for a single-card identity — Helix ``/users?login=`` is exact.
        """
        login = (username or "").strip().lstrip("@").lower()
        if not login:
            return {}
        if not self.client_id or not self.client_secret:
            return {}
        ...
        if not users:
            return {}
        user = users[0]
        model = self._offline_user_model(user, login)
        user_id = str(user.get("id") or "").strip()
        try:
            streams = await self._helix_streams_by_user_ids([user_id]) if user_id else {}
        except Exception:
            streams = {}
        stream = streams.get(user_id) if user_id else None
        if stream:
            live = self._stream_model(stream)
            # Keep Helix profile face; stream thumbnail is a live cover only.
            avatar = str(model.get("profile_image_url") or "")
            if avatar:
                live["profile_image_url"] = avatar
            model = live
        try:
            enriched = await self._enrich_models([model])
            if enriched:
                model = enriched[0]
        except Exception:
            pass
        ...
        return {
            "isOnline": is_online,
            "viewers": int(model.get("viewers") or 0) if is_online else 0,
            "followers": int(followers) if followers is not None else None,
            "channelUrl": model.get("channel_url") or self.canonical_url(login),
            "profileImageUrl": str(model.get("profile_image_url") or "").strip(),
            "displayName": model.get("display_name") or login,
            "username": model.get("username") or login,
            ...
            "userId": model.get("user_id") or user_id or None,
        }
    ...
    async def verify_logged_in(self) -> Optional[bool]:
        ...
        try:
            ...
            if not token:
                return False
            await self._twitch_current_user()
            return True
    ...
    async def sync_following(self) -> list[dict[str, object]]:
        ...
        try:
            ...
            # Web auth-token often lacks user:read:follows; GQL still works.
            items = None
        # Helix channels/followed often returns [] without user:read:follows.
        # An empty list is not "follows nobody" — fall back to GQL.
        # Helix rows also omit profile faces; prefer GQL when any face is missing.
        need_faces = bool(items) and any(
            not str(item.get("profile_image_url") or "").strip() for item in items
        )
        if not items or need_faces:
            try:
                gql_items = await self._twitch_sync_gql_followed()
            except Exception:
                gql_items = []
            if gql_items:
                items = gql_items
                ...
                items = []
        items = await self._hydrate_followed_live_status(items or [])
        items.sort(
            key=lambda item: (
    ...
    async def _twitch_device_id(self) -> str:
        cookies = await self._session_cookie_map()
        for key in ("unique_id", "unique_id_durable", "local_storage_device_id"):
            value = str(cookies.get(key) or "").strip()
            if value:
                return value
        # Stable-enough fallback for the process lifetime.
        cached = getattr(self, "_twitch_device_id_cache", "") or ""
        if cached:
            return cached
        cached = uuid.uuid4().hex
        self._twitch_device_id_cache = cached
        return cached

    async def _twitch_integrity_token(self) -> str:
        token = await self._user_oauth_token()
        device_id = await self._twitch_device_id()
        headers = {
        ...
        if status >= 400 or not isinstance(payload, dict):
            raise ProviderError(f"Twitch integrity failed ({status})")
        integrity = str(payload.get("token") or "").strip()
        if not integrity:
            raise ProviderError("Twitch integrity token missing")
        self._twitch_device_id_cache = device_id
        return integrity

    async def _twitch_gql(
        self,
        query: str,
        ...
        *,
        ...
        if require_integrity:
            headers["Client-Integrity"] = await self._twitch_integrity_token()
        timeout = aiohttp.ClientTimeout(total=20)
        ...
        if isinstance(errors, list) and errors:
            ...
            if "integrity" in lowered:
                raise ProviderError(
                    "Twitch remote follow needs Mac Helper "
                    "(integrity check failed on VPS)"
                )
    ...
    async def _twitch_user_id(self, username: str) -> str:
        ...
        try:
            ...
            pass
        # Browser session can resolve login→id via GQL without app credentials.
        try:
            data = await self._twitch_gql(
                "query HXYLIVEUserId($login: String!) { user(login: $login) { id } }",
                {"login": login},
            )
        except Exception:
            return ""
    ...
    async def _hydrate_followed_live_status(
        ...
        # When Helix app credentials are missing, fill faces via GQL user lookup.
        still_missing = [
            login for login in missing_logins
            if login and login.lower() not in users_by_login
        ]
        for login in still_missing[:40]:
            try:
                data = await self._twitch_gql(
                    """
                    query HXYLIVEUserFace($login: String!) {
                      user(login: $login) {
                        id
                        login
                        displayName
            ...
            user = data.get("user") if isinstance(data.get("user"), dict) else {}
            if user.get("login"):
                users_by_login[str(user.get("login") or "").lower()] = {
                    "id": user.get("id"),
        ...
        for item in items:
            ...
            if user.get("id") and not row.get("user_id"):
                row["user_id"] = str(user.get("id") or "")
            hydrated.append(row)
        return hydrated
    ...
    async def _twitch_follow_gql(self, username: str, *, follow: bool) -> dict[str, object]:
        ...
        await self._twitch_gql(mutation, {"id": target_id}, require_integrity=True)
        return {
            "success": True,
            "remote": True,
            "provider": "twitch",
