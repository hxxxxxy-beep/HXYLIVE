...
class YtDlpProvider(BaseProvider):
    ...
    @staticmethod
    def _is_hls_or_dash(url: str, protocol: str = "") -> bool:
        lower = (url or "").lower()
    ...
    def _select_media_url(self, info: dict, max_height: Optional[int]) -> tuple[str, dict[str, str]]:
        # Watch uses hls.js; never select FLV/HTTP progressive even if yt-dlp
        # marks it as the top-level "best" URL (common on large Bilibili rooms).
        candidates = []
        direct_url = info.get("url")
        if direct_url and self._is_hls_or_dash(direct_url, info.get("protocol") or ""):
            candidates.append({
                "url": direct_url,
        ...
        for fmt in info.get("formats") or []:
            ...
            if not url:
                continue
            if not self._is_hls_or_dash(url, fmt.get("protocol") or ""):
                continue
        ...
        def score(fmt: dict) -> tuple[int, int, int, int]:
            height = int(fmt.get("height") or 0)
            tbr = int(fmt.get("tbr") or 0)
            fits = 1 if not max_height or height <= max_height or height == 0 else 0
            vcodec = (fmt.get("vcodec") or "").lower()
            # Prefer AVC over taller HEVC — HEVC HLS often paints black in browsers.
            avc_pref = 1 if ("avc" in vcodec or not vcodec or vcodec in ("none", "unknown")) else 0
            return (fits, avc_pref, height if fits else -height, tbr)
