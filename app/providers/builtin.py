...
class ChaturbateProvider(BaseProvider):
    ...
    async def import_session(
        ...
        # Chrome cf_clearance / __cf_bm are bound to the Mac IP; keep only CTB auth
        # cookies and let FlareSolverr mint VPS clearance during validation.
        from ..services.chaturbate_auth import strip_cloudflare_cookies
        strip_cloudflare_cookies(cookie_map)
        if user_agent:
    ...
    async def verify_logged_in(self) -> Optional[bool]:
        ...
        if not cookies.get("sessionid"):
            return False
        try:
            valid = await self.auth._validate_session()
            self.auth._is_logged_in = bool(valid)
            if not valid:
                reason = (
                    getattr(self.auth, "_last_validation_error", None)
                    or getattr(self.auth, "_last_error", None)
                    or "Saved session is not logged in"
                )
                self.auth._last_error = reason
                row = await self.auth.db.get_auth_state()
                await self.auth.db.save_auth_state(
                    username=self.auth._username or (row or {}).get("username") or "",
                    password_hash=(row or {}).get("password_hash") or "",
                    is_logged_in=False,
                ...
                if self.session_store:
                    ...
                    await self.session_store.save(
                        self.source_type,
                        username=self.auth._username or (state or {}).get("username"),
                        is_logged_in=False,
                        cookies=(state or {}).get("cookies") or _cookie_list(cookies),
    ...
    async def sync_following(self) -> list[dict[str, Any]]:
        ...
        items = await self.api.get_followed_models()
        # Roomlist/HTML snapshots often only have live /ri/ thumbs. Enrich with
        # durable summary_card_image faces so Reconcile/Media can show avatars.
        try:
            await self.api._enrich_models_profile_images(list(items or []))
        except Exception:
            pass
        return items
