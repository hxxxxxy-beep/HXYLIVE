...
class FlareSolverrClient:
    ...
    async def solve_challenge(
        ...
        *,
        ...
        When ``force`` is True (e.g. after a 403), skip and clear the cookie cache
        so a stale VPS clearance cannot poison retries.
        """
        if force:
            self.invalidate_cache()

        # Check cache first — keep the full cookie jar (not only cf_clearance).
        # Browser-imported __cf_bm / related tokens are also IP-bound; a partial
        # cache update leaves Mac cookies in place and Cloudflare keeps blocking.
        if (
            not force
            and self._cached_cookies
            and time.time() < self._cache_expires_at
        ):
            logger.debug("Using cached FlareSolverr cookies")
            return {
                "cookies": dict(self._cached_cookies),
        ...
        async with self._semaphore:
            ...
            try:
                ...
                # Cache full jar (~15 min CF TTL; keep 10 min)
