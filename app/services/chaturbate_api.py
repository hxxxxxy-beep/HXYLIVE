...
from .chaturbate_auth import (
    ChaturbateAuthService,
    merge_flaresolverr_cookies,
    strip_cloudflare_cookies,
)
...
class ChaturbateAPI:
    ...
    def _apply_flaresolverr_solution(
        ...
        strip_cloudflare_cookies(merged_cookies)
    ...
    async def _prepare_flaresolverr_retry_headers(
        ...
        if not self.flaresolverr:
            return False
        # 403 already proved the cached clearance is useless for this request.
        solution = await self.flaresolverr.solve_challenge(url, force=True)
        if not solution:
            return False
                ...
                try:
                    async with session.request(
                        ...
                        **kwargs
                        ...
                        if resp.status == 403:
                            # Cloudflare block - try FlareSolverr. Login redirects
                            # must stay 302 so follow-sync can fail closed instead
                            # of treating a guest 200 as an empty follow list.
    ...
    async def check_status(self, username: str) -> Dict[str, Any]:
        """Chaturbate room status for a username via chatvideocontext (FlareSolverr-aware).
    ...
    def _lookup_model_from_chatvideocontext(
        ...
        *,
        ...
        poster = self._room_poster_url(uname)
        # Live discover covers keep the roomlist-style `riw` webcam frame.
        # Circular avatars use only summary_card_image (self-set face). Room
        # posters (/ri/) look like live screenshots — never circular faces.
        thumbnail = riw if is_online else (summary or poster or riw)
        profile_image = self._durable_profile_image(uname, summary)
        return {
            "username": uname,
            "display_name": uname,
            "is_online": is_online,
    ...
    async def lookup_username(self, username: str) -> Optional[Dict[str, Any]]:
        ...
        if resp.status == 401:
            ...
            if "password" in detail or code in {"unauthorized", "password_required", "password required"}:
                ...
                if model.get("followers") is None or int(model.get("followers") or 0) == 0:
                    total = await self._follower_count(needle)
                    if total is not None:
                        model["followers"] = total
                        model["num_followers"] = total
                    elif model.get("followers") is None:
                        model["num_followers"] = 0
                return model
            return None
        if resp.status != 200:
            return None
        ...
        # chatvideocontext often omits followers or returns 0; biocontext is authoritative.
        if model.get("followers") is None or int(model.get("followers") or 0) == 0:
            total = await self._follower_count(model["username"])
            if total is not None:
                model["followers"] = total
                model["num_followers"] = total
            elif model.get("followers") is None:
    ...
    async def get_live_models(
        ...
        try:
            ...
            # only filter is `keywords` (full-text search over tags and subject).
            # When a tag is set, concatenate it into the search keywords.
            ...
            # Stale session cookies often 302 the public roomlist to login. Discover
            # must keep working as a guest — retry once without Cookie.
            if (
                resp
                and resp.status != 200
                and headers.get("Cookie")
                and (
                    resp.status in _REDIRECT_STATUSES
                    or resp.status in {401, 403}
                )
            ):
                guest_headers = dict(headers)
                guest_headers.pop("Cookie", None)
                logger.info(
                    "Chaturbate roomlist rejected authenticated session; retrying as guest",
                    status=resp.status,
                )
                resp = await self._request(
                    "GET", api_url, headers=guest_headers, allow_redirects=False
                )

            if resp and resp.status == 200:
                parsed = self._parse_roomlist_response(resp, page, limit)
                if parsed is not None:
                    # Do not await chatvideocontext N+1 here — it made Discover
                    # "all" feel cached (slow cold path) and blocked the live
                    # roomlist. Circular avatars load via /api/discover/profile-images.
                    return parsed
            elif resp:
    ...
    @staticmethod
    def _room_poster_url(username: str) -> str:
        """Stable room poster for rectangular offline covers only.

        Looks like a live/room screenshot — never use as a circular avatar.
        Prefer the current mmcdn ``/ri/`` host; legacy ``roomimg`` redirects
        there and its TLS certificate is expired in browsers.
        """
        uname = str(username or "").strip().lower()
        if not uname:
            return ""
        return f"https://thumb.live.mmcdn.com/ri/{uname}.jpg"

    @classmethod
    def _durable_profile_image(cls, username: str, summary: object) -> str:
        face = cls._optional_image_url(summary)
        if face and not cls._is_riw_image_url(face) and not cls._is_room_poster_url(face):
            return face
        return ""

    @classmethod
    def _is_room_poster_url(cls, value: Any) -> bool:
        raw = str(value or "").strip().lower()
        if not raw:
            return False
        if "roomimg.stream.highwebmedia.com/ri/" in raw:
            return True
        # Room preview CDN (not the live /riw/ webcam frame, not a face photo).
    ...
    def _summary_card_image_from_room(cls, room: Dict[str, Any]) -> str:
        """Self-set profile photo when roomlist/chatvideocontext expose one.

        Live ``riw`` frames and ``/ri/`` room posters are covers, not circular
        avatars. Empty when Chaturbate omits ``summary_card_image``.
        """
        for key in (
            "summary_card_image",
            "summaryCardImage",
            ...
            if (
                candidate
                and not cls._is_riw_image_url(candidate)
                and not cls._is_room_poster_url(candidate)
            ):
                return candidate
        return ""
    ...
    async def _enrich_models_profile_images(self, models: List[Dict[str, Any]]) -> None:
        ...
        Room posters are never used as faces — leave empty for letter avatars.
        """
        if not models:
            return
        usernames: List[str] = []
        for model in models:
            ...
            if current and not self._is_riw_image_url(current) and not self._is_room_poster_url(current):
                continue
        ...
        for model in models:
            ...
            if not summary or self._is_riw_image_url(summary) or self._is_room_poster_url(summary):
                continue
            current = str(model.get("profile_image_url") or model.get("profileImageUrl") or "").strip()
            if current == summary:
                continue
            # Upgrade empty / live riw / room-poster placeholders to a real face.
            if (
                not current
                or self._is_riw_image_url(current)
                or self._is_room_poster_url(current)
            ):
    ...
    async def _chatvideocontext_summary_image(self, username: str) -> str:
        """Resolve a self-set circular avatar for Chaturbate cards.

        Uses chatvideocontext ``summary_card_image`` only. Never returns live
        ``riw`` frames or ``/ri/`` room posters (those look like screenshots).
        """
        uname = str(username or "").strip().strip("/").lower()
        ...
        if not isinstance(data, dict):
            return ""
        return self._durable_profile_image(uname, data.get("summary_card_image"))
    ...
    async def get_followed_models(self) -> List[Dict[str, Any]]:
        ...
        completely. An empty JSON list is not trusted. A complete HTML snapshot
        (no ambiguous pagination) is authoritative for a manual Sync follows.
        """
        status_getter = getattr(self.auth, "get_status", None)
        status = status_getter() if callable(status_getter) else None
        # Prefer a verified session, but still attempt sync when sessionid is present.
        # Transient Cloudflare/validation flaps must not block follow sync as "not logged in".
        has_session = bool(self.auth.get_cookies().get("sessionid"))
        logged_in = isinstance(status, dict) and status.get("isLoggedIn") is True
        if not has_session and not logged_in:
    ...
    async def _fetch_followed_html(self, headers: Dict[str, str]) -> FollowedSyncResult:
        ...
        while True:
            ...
            if self._looks_like_cloudflare_challenge(html):
                solved = await self._prepare_flaresolverr_retry_headers(url, html_headers)
                if solved:
                    resp = await self._request("GET", url, headers=html_headers, allow_redirects=False)
                    if not resp:
                        return self._followed_failure("Chaturbate followed-cams request failed")
                    ...
                    if resp.status != 200:
                        return self._followed_failure(
                            f"Chaturbate followed-cams returned HTTP {resp.status}"
                        )
                    html = resp.text()
            if self._looks_like_login_or_challenge(html):
                if self._looks_like_cloudflare_challenge(html):
                    return self._followed_failure(
                        "Chaturbate followed-cams returned a Cloudflare challenge"
                    )
                return self._followed_failure("Chaturbate followed-cams returned a login page")
    ...
    async def _fetch_followed_roomlist_api(self, headers: Dict[str, str]) -> FollowedSyncResult:
        ...
        if not models:
            return self._followed_failure("Chaturbate followed roomlist is empty")
    ...
    def _looks_like_cloudflare_challenge(html: str) -> bool:
        lower = (html or "").lower()
        return "cloudflare" in lower and ("challenge" in lower or "cf-browser-verification" in lower)

    @classmethod
    def _looks_like_login_or_challenge(cls, html: str) -> bool:
        if cls._looks_like_cloudflare_challenge(html):
            return True
        lower = (html or "").lower()
    ...
    def _followed_json_item(cls, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            # Roomlist often omits followers; never invent 0 (that sticks in DB via COALESCE).
            "num_followers": cls._as_optional_int(item.get("num_followers")),
        }
    ...
    def _followed_roomlist_item(cls, model: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "num_followers": cls._as_optional_int(model.get("num_followers")),
        }
    ...
    def _parse_room_item(item: Dict[str, Any], is_online: bool = True) -> Dict[str, Any]:
        ...
        if thumb and thumb.startswith("//"):
            thumb = "https:" + thumb
        if not thumb:
            thumb = f"https://thumb.live.mmcdn.com/ri/{username}.jpg"
        room_status = item.get("current_show") or item.get("room_status") or None
        profile_image = ChaturbateAPI._summary_card_image_from_room(item)
        ...
        return {
            ...
            # Missing roomlist followers must stay unknown — default 0 poisons Media cards.
            "num_followers": ChaturbateAPI._as_optional_int(item.get("num_followers")),
        }
