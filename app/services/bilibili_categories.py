...
Discover Bilibili pills use a curated allowlist (not the full Area/getList tree).
Short TTL cache. Never invents Female/Male/Trans/Couple.
Room-list filtering still uses parent_area_id from these rows.
"""
...
# Discover UI: only Virtual streamers (no All pill; that partition is the default).
# parent_area_id is the getRoomList filter; name is the English pill label.
CURATED_BILIBILI_PARENT_AREAS: Tuple[Dict[str, str], ...] = (
    {"parent_area_id": "9", "name": "Virtual streamers"},
)
CURATED_BILIBILI_PARENT_AREA_IDS = frozenset(
    row["parent_area_id"] for row in CURATED_BILIBILI_PARENT_AREAS
)
...
def curated_bilibili_parent_areas() -> List[Dict[str, str]]:
    """Stable allowlist for Discover Bilibili category pills."""
    return [dict(row) for row in CURATED_BILIBILI_PARENT_AREAS]


def is_curated_bilibili_parent_area_id(value: Any) -> bool:
    aid = normalize_bilibili_parent_area_id(value)
    return bool(aid and aid in CURATED_BILIBILI_PARENT_AREA_IDS)


def filter_bilibili_parents_to_allowlist(
    raw_items: Sequence[Dict[str, Any]],
) -> List[Dict[str, str]]:
    """Map Area/getList rows onto the curated allowlist (labels/order fixed)."""
    _ = raw_items
    return curated_bilibili_parent_areas()
...
async def list_bilibili_parent_areas(
    *,
    ...
    """Return curated Bilibili parent areas for the categories API.

    Always returns the allowlist (Virtual streamers). Area/getList is
    optional and does not expand the set.
    """
    ...
    async with _cache_lock:
        ...
        curated = curated_bilibili_parent_areas()
        if area_fetcher is not None:
            try:
                raw = await _fetch_area_list(fetcher=area_fetcher)
                _ = filter_bilibili_parents_to_allowlist(dedupe_bilibili_parent_areas(raw))
            except Exception:
                pass
    ...
    *,
    ...
    async with _cache_lock:
        ...
        try:
            ...
            # Keep parent cache warm, but never expand past the curated allowlist.
            _cache_items = curated_bilibili_parent_areas()
            _cache_expires_at = now + _CATEGORY_CACHE_TTL_SECONDS
