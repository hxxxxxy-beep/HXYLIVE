...
from .services.bilibili_categories import curated_bilibili_parent_areas
...
# Twitch game_id / Bilibili parent_area_id / YouTube live_query
# (legacy video_category_id keys still parse for URL compatibility).
# Other request_param values must not be auto-sent by the frontend.
SUPPORTED_DISCOVER_REQUEST_PARAMS = frozenset({
    "gender",
    "game_id",
    "parent_area_id",
    "video_category_id",
...
# YouTube Discover is handle-search only (no All, no content pills).
# Chaturbate / Stripchat keep All + gender.
SOURCES_WITHOUT_ALL_CATEGORY = frozenset({"twitch", "bilibili", "youtube"})
...
DEFAULT_YOUTUBE_LIVE_QUERY_NAME = "ASMR"
...
def _youtube_ranking_hints() -> Dict[str, Any]:
    modes = ["page_local"]
    return {
        "supports_viewer_count": True,
        "viewer_count_reliable": True,
        ...
        "ranking_modes": list(modes),
        "ranking_modes_available": list(modes),
        "evidence_source": "concurrentViewers",
        "implementation_status": "verified",
        "evidence_note": (
            "YouTube liveStreamingDetails.concurrentViewers; Discover stays page_local."
        ),
    }
...
_RANKING_HINTS_BY_SOURCE: Dict[str, Dict[str, Any]] = {
    ...
    "youtube": _youtube_ranking_hints(),
}
...
def _request_mapping_for(category_type: str, canonical: str) -> Dict[str, Any]:
    ...
    if ctype == CATEGORY_TYPE_CONTENT or ctype == "content":
        ...
        if key.startswith("video_category:"):
            category_id = key.split(":", 1)[1].strip()
            return {
                "request_param": "video_category_id",
                "request_value": category_id,
                "filter_scope": FILTER_SCOPE_CONTENT,
            }
        if key.startswith("live_query:"):
            live_query = key.split(":", 1)[1].strip()
            return {
                "request_param": "live_query",
                "request_value": live_query,
                "filter_scope": FILTER_SCOPE_CONTENT,
            }
        return {
...
def youtube_live_query_category_item(live_query: str, name: str) -> Dict[str, Any]:
    """Formal YouTube live-search category (no native videoCategoryId)."""
    key = str(live_query or "").strip().lower()
    label = str(name or "").strip()
    return _category_item(
        canonical=f"live_query:{key}",
        category_type=CATEGORY_TYPE_CONTENT,
        source_mode=SOURCE_MODE_STRUCTURED,
        source_values=[key],
        reliability=RELIABILITY_MEDIUM,
        source_signal_present=True,
        available=True,
        readiness=READINESS_VERIFIED,
        field_name="live_query",
        data_locus="youtube_live_query",
        upstream_parameter={
            "search": "GET /youtube/v3/search eventType=live",
            "streams_filter": "q",
        },
        evidence_note="YouTube live search query; no native ASMR videoCategoryId.",
        request_param="live_query",
        request_value=key,
        filter_scope=FILTER_SCOPE_CONTENT,
        display_label=label,
...
def is_executable_request_mapping(item: Dict[str, Any]) -> bool:
    ...
    if param_s == "parent_area_id" and not value_s.isdigit():
        return False
    if param_s == "video_category_id" and not value_s.isdigit():
        return False
    if param_s == "live_query" and not normalize_youtube_live_query(value_s):
        return False
    return True
...
def build_categories_payload(
    ...
    *,
    ...
    youtube_categories: Optional[Sequence[Dict[str, Any]]] = None,
    ...
    Bilibili parent areas use a curated Virtual streamers allowlist.
    YouTube Discover has no content-category pills (exact @handle search only).
    Twitch/Bilibili/YouTube omit All; Chaturbate/Stripchat keep All + gender.
    """
    source_key = (source or "").strip().lower()
    ...
    if source_key == "bilibili":
        for row in curated_bilibili_parent_areas():
            aid = str(row.get("parent_area_id") or "").strip()
            name = str(row.get("name") or "").strip()
    ...
    if source_key == "youtube":
        for row in list(youtube_categories or curated_youtube_video_categories()):
            if not isinstance(row, dict):
                continue
            name = str(row.get("name") or "").strip()
            live_query = normalize_youtube_live_query(row.get("live_query"))
            if not live_query or not name:
                continue
            item = youtube_live_query_category_item(live_query, name)
            item["source_type"] = source_key
            evidence.append({
                "canonical_category": item["canonical_category"],
                "source_signal_present": True,
    ...
    diagnostics = {
        ...
        "bilibili_content_source": (
            "curated_parent_areas" if source_key == "bilibili" else None
        ),
        "youtube_content_source": (
            "handle_search_only" if source_key == "youtube" else None
        ),
    }
