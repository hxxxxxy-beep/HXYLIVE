...
from ..services.bilibili_categories import (
    is_curated_bilibili_parent_area_id,
    list_bilibili_parent_areas,
    normalize_bilibili_parent_area_id,
)
from ..services.youtube_categories import (
    list_youtube_content_categories,
)
from ..providers.youtube import normalize_youtube_discover_identity
...
async def discover_categories(
    ...
    Twitch content categories come from a curated Helix game_id allowlist.
    Bilibili content categories come from a curated parent-area allowlist
    (Virtual streamers).
    YouTube Discover has no category pills (exact @handle search only).
    ...
    if source_key == "twitch":
        ...
        youtube_categories = await list_youtube_content_categories()
...
async def discover_models(
    ...
    video_category_id: Optional[str] = Query(
        None,
        description="Deprecated YouTube videoCategoryId filter; ignored",
    ),
    live_query: Optional[str] = Query(
        None,
        description="Deprecated YouTube live-query category; ignored (handle search only)",
    ),
    ...
    # YouTube Discover accepts pasted channel URLs; normalize to @handle for
    # local offline-hit filtering / ranking (provider also normalizes).
    if source_key == "youtube" and (search or "").strip():
        identity = normalize_youtube_discover_identity(search)
        if identity:
            search_lower = identity.lower()
    ...
    # YouTube Discover is exact @handle search only; category params are ignored.
    _ = video_category_id
    effective_video_category_id = None
    effective_live_query = None
    _ = live_query
    ...
    if source_key == "bilibili" and not is_curated_bilibili_parent_area_id(
    ...
    results = await asyncio.gather(*[
        _fetch_provider(
            ...
            video_category_id=effective_video_category_id,
    ...
    def _filter_list(result: Optional[Dict[str, Any]], source_type: str) -> List[Dict[str, Any]]:
        ...
        # YouTube users often type @handle; match against the bare handle.
        search_needle = search_lower.lstrip("@") if search_lower else ""
        for item in result.get("models", []):
            ...
            if search_needle:
                ...
                if source_type in {"twitch", "chaturbate", "stripchat", "youtube"}:
                    if search_needle in username_lower or search_needle in dname:
                        offline_search_hit = True
                    elif source_type == "youtube" and (
                        search_needle == uid or search_needle == room_id
                    ):
                        offline_search_hit = True
                elif source_type == "bilibili" and (
                    search_needle in dname
                    or search_needle in username_lower
                    or search_needle == uid
                    or search_needle == room_id
                    or search_needle == short_id
                ):
                    offline_search_hit = True
            ...
            if not _matches_gender_filter(item, item_tags_lower, gender):
                # Exact / offline search hits must still surface when Stripchat
                # /users payloads omit gender (common for offline models).
                dname_l = str(item.get("display_name") or "").strip().lower()
                exact_id = bool(search_needle) and (
                    username_lower == search_needle or dname_l == search_needle
                )
                unknown_gender = not _gender_tokens(
                    [
                        item.get("gender"),
                        item.get("gender_group"),
                        item.get("genderGroup"),
                ...
                if not (offline_search_hit and (exact_id or unknown_gender)):
                    continue
            ...
            if search_needle:
                ...
                if (
                    search_needle not in uname
                    and search_needle not in dname
                    and search_needle != uid
                    and search_needle != room_id
                    and search_needle != short_id
                ):
                    continue
            item = dict(item)
            item["tags"] = item_tags
