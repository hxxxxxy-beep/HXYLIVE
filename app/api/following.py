...
def _is_live_cover_face_url(value: object) -> bool:
    ...
    if "doppiocdn." in lowered and "/snapshot/" in lowered:
        return True
    if "i.ytimg.com/" in lowered or "ytimg.com/vi/" in lowered:
        return True
...
async def sync_following():
    """
    Sync every provider that exposes a verified remote following API.
    Each provider replaces only its own HXYLIVE follows (complete snapshot).
