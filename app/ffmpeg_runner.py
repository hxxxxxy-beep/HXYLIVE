...
def _is_local_ffmpeg_input(input_url: str) -> bool:
    """True for loopback HLS proxy URLs or on-disk playlist paths."""
    if _is_local_hls_proxy_url(input_url):
        return True
    if not input_url:
        return False
    ...
    try:
        ...
        return False
    # Bare filesystem paths (no scheme) used for pre-written master playlists.
    return not parsed.scheme and input_url.startswith("/")
...
def _hls_input_args(
    ...
    *,
    ...
    is_chaturbate = _is_chaturbate_hls_url(input_url) or _is_chaturbate_hls_url(source_url or "")
    if is_chaturbate:
        args.extend([
            "-http_persistent", "1",
            "-http_multiple", "0",
...
def _build_ffmpeg_command(
    ...
    local_input = _is_local_ffmpeg_input(input_url)
    filesystem_input = bool(
        local_input
        and not _is_local_hls_proxy_url(input_url)
        and not str(input_url).lower().startswith(("http://", "https://", "file:"))
    )
    if not local_input:
        # Reconnect options for stability. Local playlist files / loopback HLS
        # proxy inputs must not reconnect-at-EOF or they can stall empty.
        cmd.extend([
            "-reconnect", "1",
            "-reconnect_at_eof", "1",
        ...
        # Local masters (file path or loopback) may nest http(s) variant URLs.
        # FFmpeg defaults file: inputs to protocol_whitelist=file,crypto,data.
        cmd.extend([
            "-protocol_whitelist",
            "file,http,https,tcp,tls,crypto",
        ])

    # -user_agent is an HTTP-protocol option. On bare filesystem inputs FFmpeg 7
    # reports "Option user_agent not found" and aborts before nested fetches.
    ...
    # Never send localhost/file inputs through an outbound HTTP proxy — that
    # can black-hole loopback requests and stall the whole recording path.
    proxy_url = None if local_input else ffmpeg_http_proxy_url()
    if proxy_url:
        cmd.extend(["-http_proxy", proxy_url])
    elif not local_input and is_socks_proxy(get_outbound_proxy_url()):
        logger.warning(
    ...
    cmd.extend(
        _hls_input_args(
            ...
            allow_http_headers=not filesystem_input,
        )
    )
...
class FFmpegSession:
    def __init__(
        ...
        self._record_stop_emitted = False
    ...
    def _writer_loop(self):
        ...
        try:
            ...
            try:
                from .services.recording_audit import emit
                reason = self.stop_reason
                if not reason:
                    rc = self.exit_returncode
                    reason = "ffmpeg_exit" if rc == 0 else "ffmpeg_error"
                emit(
                    "record_stop",
                    ...
                    exitCode=self.exit_returncode,
                    reason=reason,
                )
                self._record_stop_emitted = True
            except Exception:
                pass
...
class FFmpegManager:
    ...
    def start_session(
        ...
        with self._lock:
            ...
            try:
                ...
                try:
                    from .services.recording_audit import emit, refresh_busy_snapshot
                    emit(
                        "record_start",
                        username=person,
                        sessionId=session_id,
                        path=sess.record_path_today(),
                        sourceType=source_type,
                        target=target,
                    )
                    # Must not call list_status() while holding self._lock
                    # (threading.Lock is not re-entrant → permanent deadlock).
                    refresh_busy_snapshot(self._statuses_locked())
                except Exception:
                    pass
    ...
    def stop_session(self, session_id: str, reason: str = "api_stop") -> bool:
        with self._lock:
            ...
            if not sess.stop_reason:
                sess.stop_reason = reason or "api_stop"
            ...
            if not sess._record_stop_emitted:
                try:
                    from .services.recording_audit import emit
                    emit(
                        "record_stop",
                        username=sess.person,
                        sessionId=sess.id,
                        path=sess.record_path,
                        bytesWritten=sess.bytes_written,
                        elapsedSeconds=round(duration, 1),
                        exitCode=sess.exit_returncode,
                        reason=sess.stop_reason or reason or "api_stop",
                        writerJoined=not bool(sess._writer_thread and sess._writer_thread.is_alive()),
                    )
                    sess._record_stop_emitted = True
                except Exception:
                    pass
            ...
            try:
                from .services.recording_audit import refresh_busy_snapshot
                # Must not call list_status() while holding self._lock.
                refresh_busy_snapshot(self._statuses_locked())
            except Exception:
                pass
    ...
    def stop_stalled_sessions(self, max_idle_seconds: Optional[int] = None) -> List[dict]:
        ...
        for session_id in self.stalled_session_ids(max_idle_seconds):
            ...
            if self.stop_session(session_id, reason="stall_watchdog"):
                stopped.append(info)
        return stopped

    def _statuses_locked(self) -> List[dict]:
        """Build status rows. Caller must already hold self._lock."""
        out = []
        for sess in self._sessions.values():
            out.append({
                "id": sess.id,
