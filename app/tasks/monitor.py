    ...
    async with aiohttp_client_session() as session:
        while True:
            try:
                ...
                # Media cards with sources must be polled even before a models
                # row exists — otherwise live_sessions never covers the card.
                try:
                    profile_sources = await db.get_all_media_profile_sources()
                except Exception:
                    profile_sources = []
                tracked = {
                    (
                        str(m.get("username") or "").strip().lower(),
                        (m.get("source_type") or "chaturbate"),
                    )
                    for m in models
                    if m.get("username")
                }
                for source in profile_sources:
                    ...
                    models.append({
                        ...
                        "profile_username": source.get("profile_username") or channel,
                        "thumbnail_updated_at": 0,
                        "_from_media_source": True,
                    })
                ...
                for model in models:
                    ...
                    profile_username = model.get('profile_username')
                    ...
                    try:
                        ...
                        # Ensure Media-only channels get a models row once so
                        # status + live_sessions persist. Never rewrite auto_record.
                        if model.get("_from_media_source"):
                            try:
                        ...
                        if profile_username and profile_username != username:
                            try:
                                await db.observe_live_status(
                                    username,
                                    is_online=bool(status['is_online']),
                                    source_type=source_type,
                                    profile_username=str(profile_username),
                                    detection="monitor",
                                )
                            except Exception:
                                pass
