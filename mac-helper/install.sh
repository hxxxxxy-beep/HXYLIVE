...
Examples:
  ...
  ./mac-helper/install.sh --origin http://203.0.113.10:8080 \
      ...
      --chrome-download-dir "$HOME/Downloads"
EOF
}
...
case "$VIDEO_DIR" in
    /Volumes/*)
        VOLUME_NAME="${VIDEO_DIR#/Volumes/}"
        VOLUME_NAME="${VOLUME_NAME%%/*}"
        if [ -d "/Volumes/$VOLUME_NAME" ]; then
            mkdir -p "$VIDEO_DIR"
        else
            printf 'Video directory volume is not mounted; not creating %s\n' "$VIDEO_DIR"
        fi
        ;;
    *)
        mkdir -p "$VIDEO_DIR"
...
case "$VIDEO_DIR" in
    /Volumes/*)
        ...
        printf 'Unplugging that disk hides local videos until it is remounted.\n'
        ;;
esac
